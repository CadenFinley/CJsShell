...
int ShellScriptInterpreter::execute_block(const std::vector<std::string>& lines) {
  ...
  execute_simple_or_pipeline = [&](const std::string& cmd_text) -> int {
    ...
    // Expand command substitution $(...) and arithmetic $((...)) without using external sh
  auto capture_internal_output = [&](const std::string& content) -> std::string {
      ...
      auto append_redirect = [&](const std::string& segment) -> int {
        ...
        for (size_t i = 0; i < segment.size(); ++i) {
          ...
          if ((c=='"' || c=='\'') && (i==0 || segment[i-1] != '\\')) {
            ...
            else if (q==c) { in_quotes=false; q='\0'; }
          }
          if (!in_quotes && c == '>') { has_stdout_redir = true; break; }
        }
      ...
      std::vector<LogicalCommand> lcmds = shell_parser->parse_logical_commands(content);
      int rc = 0;
  ...
  ::unlink(path.c_str());
      ...
      // Fallback: if empty, try executing with /bin/sh -c and capture stdout via popen
      if (out.empty()) {
        ...
        if (fp) {
          ...
          std::string tmpOut;
          while ((n = fread(buf, 1, sizeof(buf), fp)) > 0) tmpOut.append(buf, n);
          pclose(fp);
          while (!tmpOut.empty() && (tmpOut.back() == '\n' || tmpOut.back() == '\r')) tmpOut.pop_back();
          out = tmpOut;
        }
      }
      return out;
    ...
    auto eval_arith = [&](const std::string& expr) -> long long {
      ...
      struct Tok { enum T{NUM,OP,LP,RP} t; long long v; char op; };
      auto is_space = [](char c){ return c==' '||c=='\t'||c=='\n'||c=='\r'; };
      auto prec = [](char op){ if(op=='+'||op=='-') return 1; if(op=='*'||op=='/'||op=='%') return 2; return 0; };
      auto apply = [](long long a, long long b, char op)->long long{ switch(op){case '+':return a+b;case '-':return a-b;case '*':return a*b;case '/':return b==0?0:a/b;case '%':return b==0?0:a%b;default:return 0;}};
      std::vector<Tok> output; std::vector<char> ops;
      // Tokenize
      for (size_t i=0;i<expr.size();) {
        if (is_space(expr[i])) { ++i; continue; }
        if (isdigit(expr[i])) {
          long long val=0; size_t j=i; while (j<expr.size() && isdigit(expr[j])) { val = val*10 + (expr[j]-'0'); ++j; }
          output.push_back({Tok::NUM,val,0}); i=j; continue;
        }
        if (isalpha(expr[i]) || expr[i]=='_') {
          size_t j=i; while (j<expr.size() && (isalnum(expr[j]) || expr[j]=='_')) ++j;
          std::string name = expr.substr(i,j-i);
          const char* ev = getenv(name.c_str());
          long long val = 0; if (ev) { try { val = std::stoll(ev); } catch (...) { val = 0; } }
          output.push_back({Tok::NUM,val,0}); i=j; continue;
        }
        if (expr[i]=='(') { ops.push_back('('); ++i; continue; }
        if (expr[i]==')') { while(!ops.empty() && ops.back()!='('){ char op=ops.back(); ops.pop_back(); output.push_back({Tok::OP,0,op}); } if(!ops.empty()&&ops.back()=='(') ops.pop_back(); ++i; continue; }
        char op = expr[i];
        if (op=='+'||op=='-'||op=='*'||op=='/'||op=='%') {
          while(!ops.empty() && ops.back()!='(' && prec(ops.back())>=prec(op)) { char top=ops.back(); ops.pop_back(); output.push_back({Tok::OP,0,top}); }
          ops.push_back(op); ++i; continue;
        }
        // Unknown char, skip
        ++i;
      }
      while(!ops.empty()){ char op=ops.back(); ops.pop_back(); if(op!='(') output.push_back({Tok::OP,0,op}); }
      // Eval RPN
      std::vector<long long> st;
      for (auto& t: output){ if(t.t==Tok::NUM) st.push_back(t.v); else if(t.t==Tok::OP){ if(st.size()<2){ st.clear(); break;} long long b=st.back(); st.pop_back(); long long a=st.back(); st.pop_back(); st.push_back(apply(a,b,t.op)); } }
    ...
    auto expand_substitutions = [&](const std::string& in) -> std::string {
      ...
      while (changed) {
        ...
        for (size_t i = 0; i + 2 < s.size(); ++i) {
          ...
          if ((!in_quotes || q == '"') && c == '$' && s[i+1] == '(' && s[i+2] == '(') {
            // find matching )) with parenthesis depth
            size_t j = i + 3; int depth = 1; bool found = false; bool in_q=false; char qq='\0';
            for (; j < s.size(); ++j) {
              char d = s[j];
              if ((d == '"' || d == '\'') && (j == i+3 || s[j-1] != '\\')) {
                if (!in_q) { in_q = true; qq = d; }
                else if (qq == d) { in_q = false; qq = '\0'; }
              } else if (!in_q) {
                if (d == '(') depth++;
                else if (d == ')') { depth--; if (depth == 0) { if (j+1 < s.size() && s[j+1] == ')') { found = true; j++; } break; } }
              }
            }
            if (found) {
      ...
      // Then, $( ... ) command substitution. Skip $(( which was already handled.
      changed = true;
    while (changed) {
        changed = false;
        bool in_quotes = false; char q = '\0';
      ...
      if ((!in_quotes || q == '"') && c == '$' && s[i+1] == '(') {
            if (i + 2 < s.size() && s[i+2] == '(') continue; // arithmetic already
            // find matching ) with nesting
            size_t j = i + 2; int depth = 1; bool found = false; bool in_q=false; char qq='\0';
            for (; j < s.size(); ++j) {
              char d = s[j];
              if ((d == '"' || d == '\'') && (j == i+2 || s[j-1] != '\\')) {
                if (!in_q) { in_q = true; qq = d; }
                ...
                if (d == '(') depth++;
                else if (d == ')') { depth--; if (depth == 0) { found = true; break; } }
              }
            }
            if (found) {
  ...
  auto split_ampersand = [&](const std::string& s) -> std::vector<std::string> {
    ...
    for (size_t i = 0; i < s.size(); ++i) {
      ...
      if ((c == '"' || c == '\'') && (i == 0 || s[i-1] != '\\')) {
        ...
        // If it's a double ampersand, leave for logical splitting (already handled earlier)
        if (i + 1 < s.size() && s[i+1] == '&') {
  ...
  auto handle_if_block = [&](const std::vector<std::string>& src_lines, size_t& idx) -> int {
    ...
    if (pos != std::string::npos) {
      ...
      while (!then_found && ++j < src_lines.size()) {
        ...
        if (cur == "then") { then_found = true; break; }
    ...
    int cond_rc = cond_accum.empty() ? 1 : execute_simple_or_pipeline(cond_accum);
    ...
    if (pos != std::string::npos) {
      ...
      std::string body = fi_pos == std::string::npos ? rem : trim(rem.substr(0, fi_pos));
      ...
      if (cond_rc == 0) {
        ...
        for (const auto& c : cmds) { int rc2 = execute_simple_or_pipeline(c); body_rc = rc2; if (rc2 != 0) break; }
    ...
    while (k < src_lines.size() && depth > 0) {
      ...
      if (cur == "if") depth++;
      else if (cur == "fi") { depth--; if (depth == 0) break; }
      else if (depth == 1 && cur == "else") { in_else = true; k++; continue; }

      if (depth > 0) {
        if (!in_else) then_lines.push_back(cur_raw);
        else else_lines.push_back(cur_raw);
      }
      k++;
    }
    if (depth != 0) {
    ...
    int body_rc = 0;
    if (cond_rc == 0) body_rc = execute_block(then_lines);
    else if (!else_lines.empty()) body_rc = execute_block(else_lines);
  ...
  auto handle_for_block = [&](const std::vector<std::string>& src_lines, size_t& idx) -> int {
    ...
    if (first.find("; do") != std::string::npos) {
      ...
      for (const auto& it : items) {
        ...
        for (const auto& c : cmds) { rc = execute_simple_or_pipeline(c); if (rc != 0) break; }
        if (rc != 0) break;
      }
      return rc;
    }
    ...
    while (!have_do && ++j < src_lines.size()) {
      std::string cur = trim(strip_inline_comment(src_lines[j]));
      if (cur == "do") { have_do = true; break; }
      if (cur.find("; do") != std::string::npos) { header_accum += " "; header_accum += cur; have_do = true; break; }
      if (!cur.empty()) { header_accum += " "; header_accum += cur; }
    }
    if (!parse_header(header_accum)) { idx = j; return 1; }
    if (!have_do) { idx = j; return 1; }
    ...
    while (k < src_lines.size() && depth > 0) {
      std::string cur_raw = src_lines[k];
      std::string cur = trim(strip_inline_comment(cur_raw));
      if (cur == "for" || cur.rfind("for ", 0) == 0) depth++;
      else if (cur == "done") { depth--; if (depth == 0) break; }
      if (depth > 0) body_lines.push_back(cur_raw);
      k++;
    }
    if (depth != 0) { idx = k; return 1; }
  ...
  auto handle_while_block = [&](const std::vector<std::string>& src_lines, size_t& idx) -> int {
    ...
    auto parse_cond_from = [&](const std::string& s, std::string& cond, bool& inline_do, std::string& body_inline) {
      inline_do = false; body_inline.clear(); cond.clear();
      std::string tmp = s;
      if (tmp == "while") { return true; }
      if (tmp.rfind("while ", 0) == 0) tmp = tmp.substr(6);
      size_t do_pos = tmp.find("; do");
      if (do_pos != std::string::npos) { cond = trim(tmp.substr(0, do_pos)); inline_do = true; body_inline = trim(tmp.substr(do_pos + 4)); return true; }
      if (tmp == "do") { inline_do = true; return true; }
      cond = trim(tmp);
      return true;
    };

    std::string cond; bool inline_do = false; std::string body_inline;
    parse_cond_from(first, cond, inline_do, body_inline);
    size_t j = idx;
    if (!inline_do) {
      while (++j < src_lines.size()) {
        std::string cur = trim(strip_inline_comment(src_lines[j]));
        if (cur == "do") { inline_do = true; break; }
        if (cur.find("; do") != std::string::npos) { parse_cond_from(cur, cond, inline_do, body_inline); break; }
        if (!cur.empty()) { if (!cond.empty()) cond += " "; cond += cur; }
      }
    }
    if (!inline_do) { idx = j; return 1; }
  ...
  std::vector<std::string> body_lines;
    if (!body_inline.empty()) {
      ...
      while (k < src_lines.size() && depth > 0) {
        std::string cur_raw = src_lines[k];
        std::string cur = trim(strip_inline_comment(cur_raw));
        if (cur == "while" || cur.rfind("while ", 0) == 0) depth++;
        else if (cur == "done") { depth--; if (depth == 0) break; }
        if (depth > 0) body_lines.push_back(cur_raw);
        k++;
      }
    ...
    int rc = 0; int guard = 0; const int GUARD_MAX = 100000;
    while (true) {
      int c = cond.empty() ? 0 : execute_simple_or_pipeline(cond);
      if (c != 0) break;
      rc = execute_block(body_lines);
      if (rc != 0) break;
      if (++guard > GUARD_MAX) { std::cerr << "cjsh: while loop aborted (guard)" << std::endl; rc = 1; break; }
    }
    return rc;
  };
  ...
  for (size_t line_index = 0; line_index < lines.size(); ++line_index) {
    ...
    if (line == "if" || line.rfind("if ", 0) == 0 || line.find("; then") != std::string::npos) {
    ...
    for (size_t i = 0; i < lcmds.size(); ++i) {
      ...
      // Split each logical segment further by semicolons, then single '&' separators
      auto semis = shell_parser->parse_semicolon_commands(lc.command);
      if (semis.empty()) { last_code = 0; continue; }
      for (size_t k = 0; k < semis.size(); ++k) {
        ...
        for (size_t si = 0; si < segs.size(); ++si) {
          ...
          if ((t.rfind("for ", 0) == 0 || t == "for") && t.find("; do") != std::string::npos) {
            size_t local_idx = 0; std::vector<std::string> one{t};
            int code = handle_for_block(one, local_idx);
            last_code = code; if (code != 0 && debug_level >= DebugLevel::BASIC) { std::cerr << "DEBUG: for block failed (" << code << ") -> '" << t << "'" << std::endl; }
            continue;
          }
          if ((t.rfind("while ", 0) == 0 || t == "while") && t.find("; do") != std::string::npos) {
            size_t local_idx = 0; std::vector<std::string> one{t};
            int code = handle_while_block(one, local_idx);
            last_code = code; if (code != 0 && debug_level >= DebugLevel::BASIC) { std::cerr << "DEBUG: while block failed (" << code << ") -> '" << t << "'" << std::endl; }
            continue;
          }
          // Case B: header/body/end split across separate semicolon segments in same line
          if ((t.rfind("for ", 0) == 0 || t == "for")) {
            // Parse header
            std::string var; std::vector<std::string> items;
            auto parse_for_header = [&](const std::string& header)->bool{
              std::vector<std::string> toks = shell_parser->parse_command(header);
              size_t i = 0; if (i < toks.size() && toks[i] == "for") ++i; if (i >= toks.size()) return false; var = toks[i++];
              if (i < toks.size() && toks[i] == "in") { ++i; for (; i < toks.size(); ++i) items.push_back(toks[i]); }
              return !var.empty();
            };
            if (!parse_for_header(t)) { last_code = 1; break; }
            ...
            if (done_index < semis.size()) {
              ...
              if (next.rfind("do ", 0) == 0) body_inline = trim(next.substr(3));
              else if (next == "do") body_inline = "";
              else { last_code = 1; break; }
              ...
              for (; scan < semis.size(); ++scan) {
                ...
                if (seg == "done") { found_done = true; break; }
                // Allow accumulating more body from subsequent segments if no 'done' yet
                if (!body_inline.empty()) body_inline += "; ";
                body_inline += seg;
              }
              if (!found_done) { last_code = 1; break; }
              // Execute loop
              int rc2 = 0;
              for (const auto& itv : items) {
                setenv(var.c_str(), itv.c_str(), 1);
                auto cmds2 = shell_parser->parse_semicolon_commands(body_inline);
                for (const auto& c2 : cmds2) { rc2 = execute_simple_or_pipeline(c2); if (rc2 != 0) break; }
                if (rc2 != 0) break;
              }
              last_code = rc2;
          ...
          if ((t.rfind("while ", 0) == 0 || t == "while")) {
            ...
            std::string cond = (t == "while") ? std::string("") : trim(t.substr(6));
            // Next segment should start with 'do'
            size_t done_index = k + 1; std::string body_inline;
            if (done_index < semis.size()) {
              std::string next = trim(strip_inline_comment(semis[done_index]));
              if (next.rfind("do ", 0) == 0) body_inline = trim(next.substr(3));
              else if (next == "do") body_inline.clear();
              else { last_code = 1; break; }
              // Collect until 'done'
              size_t scan = done_index + 1; bool found_done = false;
              for (; scan < semis.size(); ++scan) {
                std::string seg = trim(strip_inline_comment(semis[scan]));
                if (seg == "done") { found_done = true; break; }
                if (!body_inline.empty()) body_inline += "; ";
              ...
              int rc2 = 0; int guard = 0; const int GUARD_MAX = 100000;
              while (true) {
                ...
                for (const auto& c2 : cmds2) { rc2 = execute_simple_or_pipeline(c2); if (rc2 != 0) break; }
                if (rc2 != 0) break;
                if (++guard > GUARD_MAX) { std::cerr << "cjsh: while loop aborted (guard)" << std::endl; rc2 = 1; break; }
              }
              last_code = rc2;
              k = scan; // skip to 'done'
              break;
