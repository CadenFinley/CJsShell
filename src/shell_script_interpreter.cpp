...
void ShellScriptInterpreter::debug_print(const std::string& message,
...
void ShellScriptInterpreter::dump_variables() const {
  if (g_debug_mode)
    std::cerr << "DEBUG: Dumping script interpreter variables" << std::endl;
  ...
  for (const auto& pair : local_variables) {
    ss << get_indentation() << "  " << std::left << std::setw(max_length + 2)
       << pair.first << "= \"" << pair.second << "\"" << std::endl;
  }

  const char* env_vars[] = {"PATH",
                            "HOME",
                            "USER",
                            "SHELL",
                            "PWD",
...
std::string ShellScriptInterpreter::escape_debug_string(
    const std::string& input) const {
  if (g_debug_mode)
    std::cerr << "DEBUG: Escaping debug string of length " << input.length()
              << std::endl;
...
bool ShellScriptInterpreter::handle_debug_command(const std::string& command) {
  if (g_debug_mode)
    std::cerr << "DEBUG: Handling debug command: " << command << std::endl;
  ...
  if (command == "debug on") {
    ...
    std::cerr << "Current debug level: " << static_cast<int>(debug_level)
              << std::endl;
    return true;
    ...
    debug_print(
        "Enabling safe mode - path operations will be more carefully validated",
        DebugLevel::BASIC);
...
bool ShellScriptInterpreter::execute_script(const std::string& filename) {
  ...
  if (debug_level >= DebugLevel::VERBOSE) {
    debug_print("Script content (" + std::to_string(lines.size()) + " lines):",
                DebugLevel::VERBOSE);
...
bool ShellScriptInterpreter::execute_block(
  ...
  for (auto it = lines_copy.begin(); it != lines_copy.end();) {
    ...
    if (trimmed.empty() || trimmed[0] == '#') {
      debug_print("Skipping empty line or comment: " + escape_debug_string(*it),
                  DebugLevel::VERBOSE);
      ++it;
      continue;
    }
    ...
    if (!execute_line(trimmed)) {
      debug_print("Command failed: " + escape_debug_string(trimmed),
                  DebugLevel::BASIC);
      return false;
    }
...
bool ShellScriptInterpreter::execute_line(const std::string& line) {
  ...
  try {
    debug_print("Executing line: " + escape_debug_string(trimmed),
                DebugLevel::BASIC);
    ...
    if (equals_pos != std::string::npos &&
      ...
      debug_print("Variable assignment: " + var_name + "=" + var_value,
                  DebugLevel::VERBOSE);
      ...
      if (orig_value != var_value && debug_level >= DebugLevel::VERBOSE) {
        debug_print("Expanded value: " + orig_value + " -> " + var_value,
                    DebugLevel::VERBOSE);
      }

      if (var_name == "PATH" && var_value.empty()) {
        debug_print(
            "WARNING: Attempt to set PATH to empty string, using default value "
            "instead",
            DebugLevel::BASIC);
      ...
      if (is_export) {
        try {
          ...
          debug_print("Set environment variable: " + var_name + "=" + var_value,
                      DebugLevel::VERBOSE);
        } catch (const std::exception& e) {
          debug_print("ERROR: Failed to set environment variable: " + var_name +
                          " - " + e.what(),
                      DebugLevel::BASIC);
          return false;
        }
    ...
    if (trimmed.find("if ") == 0) {
      ...
      if (then_pos != std::string::npos) {
        ...
        debug_print(
            "Condition result: " + std::string(result ? "true" : "false"),
            DebugLevel::VERBOSE);
    ...
    if (trimmed.find("eval ") == 0) {
      ...
      debug_print("Eval command: " + escape_debug_string(cmd),
                  DebugLevel::VERBOSE);

      if (cmd.front() == '`' && cmd.back() == '`') {
        cmd = cmd.substr(1, cmd.size() - 2);
        debug_print("Backtick command: " + escape_debug_string(cmd),
                    DebugLevel::VERBOSE);
        ...
        try {
          ...
          if (expanded_cmd != cmd && debug_level >= DebugLevel::VERBOSE) {
            debug_print(
                "Expanded command: " + escape_debug_string(expanded_cmd),
                DebugLevel::VERBOSE);
          }
          ...
          debug_print("Command result: " + escape_debug_string(result),
                      DebugLevel::VERBOSE);
          ...
          debug_print("ERROR in eval command: " + std::string(e.what()),
                      DebugLevel::BASIC);
          return false;
        }
      }
      ...
      try {
        std::string expanded_cmd = expand_variables(cmd);
        if (expanded_cmd != cmd && debug_level >= DebugLevel::VERBOSE) {
          debug_print("Expanded eval command: " + expanded_cmd,
                      DebugLevel::VERBOSE);
        }
    ...
    debug_print("ERROR: Exception in execute_line: " + std::string(e.what()),
                DebugLevel::BASIC);
    return false;
  }
}
...
bool ShellScriptInterpreter::evaluate_condition(const std::string& condition) {
  debug_print("Evaluating condition: " + escape_debug_string(condition),
              DebugLevel::VERBOSE);
  ...
  if (condition.front() == '[' && condition.back() == ']') {
    ...
    if (test_condition.find("-x ") == 0) {
      std::string file_path =
          expand_variables(trim_string(test_condition.substr(3)));
      debug_print("Checking if file is executable: " + file_path,
                  DebugLevel::VERBOSE);
      bool result = access(file_path.c_str(), X_OK) == 0;
      debug_print("Result: " + std::string(result ? "true" : "false"),
                  DebugLevel::VERBOSE);
      return result;
    }
    ...
    if (test_condition.find("-r ") == 0) {
      std::string file_path =
          expand_variables(trim_string(test_condition.substr(3)));
      debug_print("Checking if file is readable: " + file_path,
                  DebugLevel::VERBOSE);
    ...
    if (eq_pos != std::string::npos) {
      std::string lhs =
          expand_variables(trim_string(test_condition.substr(0, eq_pos)));
      std::string rhs =
          expand_variables(trim_string(test_condition.substr(eq_pos + 3)));
      debug_print("Comparing strings: '" + lhs + "' = '" + rhs + "'",
                  DebugLevel::VERBOSE);
      bool result = lhs == rhs;
      debug_print("Result: " + std::string(result ? "true" : "false"),
                  DebugLevel::VERBOSE);
    ...
    if (neq_pos != std::string::npos) {
      std::string lhs =
          expand_variables(trim_string(test_condition.substr(0, neq_pos)));
      std::string rhs =
          expand_variables(trim_string(test_condition.substr(neq_pos + 4)));
      debug_print("Comparing strings: '" + lhs + "' != '" + rhs + "'",
                  DebugLevel::VERBOSE);
      bool result = lhs != rhs;
      debug_print("Result: " + std::string(result ? "true" : "false"),
    ...
    if (test_condition.find("-n ") == 0) {
      std::string value =
          expand_variables(trim_string(test_condition.substr(3)));
      debug_print("Checking if string is non-empty: '" + value + "'",
                  DebugLevel::VERBOSE);
    ...
    if (test_condition.find("-z ") == 0) {
      std::string value =
          expand_variables(trim_string(test_condition.substr(3)));
      debug_print("Checking if string is empty: '" + value + "'",
                  DebugLevel::VERBOSE);
      bool result = value.empty();
      debug_print("Result: " + std::string(result ? "true" : "false"),
                  DebugLevel::VERBOSE);
    ...
    debug_print("Unrecognized test condition: " + test_condition,
                DebugLevel::BASIC);
    return false;
  }

  std::string expanded_condition = expand_variables(condition);
  debug_print("Executing condition command: " + expanded_condition,
              DebugLevel::VERBOSE);
  ...
  debug_print("Command returned: " + std::to_string(status) +
                  ", result: " + std::string(result ? "true" : "false"),
              DebugLevel::VERBOSE);
  return result;
}

bool ShellScriptInterpreter::parse_conditional(
  ...
  debug_print("Parsing conditional: " + escape_debug_string(line),
              DebugLevel::VERBOSE);
  ...
  if (then_pos != std::string::npos) {
    ...
    debug_print(
        "Extracted condition from same line: " + escape_debug_string(condition),
        DebugLevel::VERBOSE);
    condition_met = evaluate_condition(condition);
    in_then_block = true;
  } else {
    condition = trim_string(line.substr(3));
    debug_print("Extracted condition from separate line: " +
                    escape_debug_string(condition),
                DebugLevel::VERBOSE);
    condition_met = evaluate_condition(condition);
    in_then_block = false;
  }
  ...
  debug_print(
      "Condition result: " + std::string(condition_met ? "true" : "false"),
      DebugLevel::VERBOSE);
  ...
  while (it != end) {
    ...
    if (in_then_block && executing_block) {
      debug_print("Adding line to conditional block: " +
                      escape_debug_string(current_line),
                  DebugLevel::TRACE);
      current_block.push_back(current_line);
    } else {
      debug_print("Skipping line in inactive block: " +
                      escape_debug_string(current_line),
                  DebugLevel::TRACE);
    }
...
std::string ShellScriptInterpreter::expand_variables(const std::string& str) {
  ...
  debug_print("Expanding variables in: " + escape_debug_string(str),
              DebugLevel::TRACE);
  ...
  while ((pos = result.find("$(", pos)) != std::string::npos) {
    ...
    if (depth == 0) {
      ...
      debug_print(
          "Processed backtick output: " + escape_debug_string(processed_output),
          DebugLevel::VERBOSE);
  ...
  while ((pos = result.find('`', pos)) != std::string::npos) {
    ...
    if (end_pos == std::string::npos) {
      debug_print(
          "Warning: Unmatched backtick at position " + std::to_string(pos),
          DebugLevel::BASIC);
      break;
    }
    ...
    if (expanded_cmd != cmd) {
      debug_print(
          "Expanded backtick command: " + escape_debug_string(expanded_cmd),
          DebugLevel::VERBOSE);
    }
  ...
  if (result.length() == 1) {
    char c = result[0];
    if (c == '\n' || c == '\r' || (c < 32 && c != '\t')) {
      debug_print("Filtered control character in expansion: " +
                      escape_debug_string(result),
                  DebugLevel::VERBOSE);
      return "";
    }
  }

  debug_print("Expanded result: " + escape_debug_string(result),
              DebugLevel::TRACE);
  return result;
}

void ShellScriptInterpreter::set_command_executor(
    std::function<bool(const std::string&, bool)> executor) {
  command_executor = executor;
}

std::string ShellScriptInterpreter::execute_command_substitution(
  ...
  debug_print("Command substitution: " + escape_debug_string(cmd),
              DebugLevel::VERBOSE);

  if (cmd.find("/usr/libexec/path_helper") != std::string::npos) {
    debug_print("Executing path_helper command with extra caution",
                DebugLevel::VERBOSE);
  }
  ...
  try {
    ...
    if (!pipe) {
      std::string error_msg =
          "Error executing command: " + escape_debug_string(cmd);
    ...
    if (status != 0) {
      debug_print("Command returned non-zero status: " + std::to_string(status),
                  DebugLevel::VERBOSE);
    }

    if (result.empty() && cmd.find("path_helper") != std::string::npos) {
      debug_print("WARNING: path_helper command returned empty result",
                  DebugLevel::BASIC);
      return "PATH=\"/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin\"; export "
             "PATH;";
    }
    ...
    debug_print("Command substitution result: " + escape_debug_string(result),
                DebugLevel::VERBOSE);
    return result;
  } catch (const std::exception& e) {
    debug_print("ERROR in command substitution: " + std::string(e.what()),
                DebugLevel::BASIC);
    if (pipe != nullptr) {
      pclose(pipe);
    }
...
std::string ShellScriptInterpreter::capture_command_output(
    const std::string& cmd) {
  ...
  debug_print("Captured output: " + escape_debug_string(result),
              DebugLevel::VERBOSE);
  return result;
}
...
std::vector<std::string> ShellScriptInterpreter::split_command(
    const std::string& cmd) {
