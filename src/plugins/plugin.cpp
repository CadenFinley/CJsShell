...
Plugin::~Plugin() {
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: Plugin destructor - Plugins disabled, "
                         "skipping cleanup"
                      << std::endl;
        }
        return;
    ...
    for (auto& [name, data] : loaded_plugins) {
        if (data.enabled && data.shutdown) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: Plugin destructor - Shutting down plugin: "
                          << name << std::endl;
            }
            data.shutdown();
        }
        if (data.handle) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: Plugin destructor - Closing handle for plugin: "
                    << name << std::endl;
            }
            dlclose(data.handle);
        }
...
bool Plugin::discover_plugins() {
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: discover_plugins - Plugins disabled, skipping "
                         "discovery"
                      << std::endl;
        }
        return false;
...

    {
        ...
        if (plugins_discovered && !loaded_plugins.empty()) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: discover_plugins - Plugins already "
                             "discovered, count: "
...

    {
        ...
        if (g_debug_mode && !loaded_plugins.empty()) {
            std::cerr
                << "DEBUG: discover_plugins - Cleaning up existing plugins "
                   "before rediscovery"
                << std::endl;
        }
        for (auto& [name, data] : loaded_plugins) {
            if (data.enabled && data.shutdown) {
                if (g_debug_mode) {
                    std::cerr
                        << "DEBUG: discover_plugins - Shutting down plugin: "
                        << name << std::endl;
                }
                data.shutdown();
            }
            if (data.handle) {
                if (g_debug_mode) {
                    std::cerr << "DEBUG: discover_plugins - Closing handle for "
                                 "plugin: "
                              << name << std::endl;
                }
                dlclose(data.handle);
...
bool Plugin::load_plugin(const std::filesystem::path& path) {
    ...
    if (!is_architecture_compatible(file_arch, current_arch)) {
        print_error(
            {ErrorType::RUNTIME_ERROR,
             ...
             {}});
        if (g_debug_mode) {
            std::cerr << "DEBUG: load_plugin - Architecture mismatch, cannot "
                         "load plugin"
                      << std::endl;
        }
        return false;
    ...
    if (dlsym_error) {
        print_error({ErrorType::RUNTIME_ERROR,
                     "plugin",
                     "Cannot load symbol 'plugin_get_info': " +
                         std::string(dlsym_error),
                     {}});
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: load_plugin - Failed to find plugin_get_info symbol"
                << std::endl;
        }
        dlclose(handle);
        return false;
        ...
        if (loaded_plugins.find(name) != loaded_plugins.end()) {
            print_error(
                {ErrorType::RUNTIME_ERROR,
                 ...
                 {}});
            if (g_debug_mode) {
                std::cerr << "DEBUG: load_plugin - Plugin already loaded, "
                             "ignoring duplicate"
                          << std::endl;
            }
            dlclose(handle);
    ...
    data.shutdown = reinterpret_cast<plugin_shutdown_func>(
    ...
    if (!data.initialize || !data.shutdown || !data.handle_command ||
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: load_plugin - Plugin missing required functions"
                << std::endl;
        }
        dlclose(handle);
        return false;
    ...
    if (data.get_default_settings) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: load_plugin - Loading default settings"
                      << std::endl;
        }
        int count = 0;
        plugin_setting_t* settings = data.get_default_settings(&count);
        if (g_debug_mode) {
            std::cerr << "DEBUG: load_plugin - Found " << count
        ...
        if (settings && count > 0) {
            for (int i = 0; i < count; i++) {
                if (settings[i].key && settings[i].value) {
                    ...
                    if (g_debug_mode) {
                        std::cerr << "DEBUG: load_plugin - Setting "
    ...
    if (data.validate) {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: load_plugin - Plugin validation passed"
                      << std::endl;
        }
    }
...
bool Plugin::extract_plugin_metadata(const std::filesystem::path& path,
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: extract_plugin_metadata - Extracting metadata from: "
            << path.filename().string() << std::endl;
    }
    ...
    if (!temp_handle) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: extract_plugin_metadata - Failed to open library: "
                << dlerror() << std::endl;
        }
    ...
    if (!info) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: extract_plugin_metadata - plugin_get_info "
                         "returned NULL"
                      << std::endl;
        }
        dlclose(temp_handle);
...
bool Plugin::load_plugin_on_demand(const std::string& name) {
...

    {
        ...
        if (loaded_plugins.find(name) != loaded_plugins.end()) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: load_plugin_on_demand - Plugin already loaded"
                    << std::endl;
            }
            return true;
        }
    ...
    if (metadata_it == plugin_metadata_cache.end()) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: load_plugin_on_demand - Plugin metadata not found"
                << std::endl;
        }
        return false;
    }
...
void Plugin::cache_plugin_metadata() {
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: cache_plugin_metadata - Plugins disabled, skipping"
                << std::endl;
        }
        return;
    }
    ...
    if (!std::filesystem::exists(plugins_directory)) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: cache_plugin_metadata - Plugin directory does "
                         "not exist: "
                      << plugins_directory << std::endl;
        }
        return;
        ...
        if (entry.path().extension() == ".so" ||
            ...
            if (existing != plugin_metadata_cache.end()) {
                if (!is_metadata_stale(existing->second)) {
                    if (g_debug_mode) {
                        std::cerr << "DEBUG: cache_plugin_metadata - Using "
                                     "cached metadata for: "
                                  << filename << std::endl;
                    }
                    continue;
            ...
            if (extract_plugin_metadata(entry.path(), metadata)) {
                ...
                std::cerr << "DEBUG: cache_plugin_metadata - Failed to extract "
                             "metadata for: "
                          << filename << std::endl;
            }
        }
...
bool Plugin::uninstall_plugin(const std::string& name) {
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: uninstall_plugin - Attempting to uninstall plugin: "
            << name << std::endl;
    }

    if (!enabled) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: uninstall_plugin - Plugins disabled, cannot "
                         "uninstall"
                      << std::endl;
        }
        return false;
    ...
    if (it == loaded_plugins.end()) {
        print_error({ErrorType::COMMAND_NOT_FOUND,
                     ...
                     {}});
        if (g_debug_mode) {
            std::cerr << "DEBUG: uninstall_plugin - Plugin not found in "
                         "loaded_plugins"
                      << std::endl;
        }
        return false;
    ...
    if (it->second.enabled) {
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: uninstall_plugin - Cannot uninstall enabled plugin, "
                   "disable first"
                << std::endl;
        }
        return false;
    ...
    for (const auto& entry :
        ...
        if (!temp_handle) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: uninstall_plugin - Failed to open library: "
                    << dlerror() << std::endl;
            }
            continue;
        }
        ...
        if (!get_info) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: uninstall_plugin - Failed to find "
                             "plugin_get_info symbol"
                          << std::endl;
            }
            dlclose(temp_handle);
        ...
        if (temp_info && std::string(temp_info->name) == name) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: uninstall_plugin - Found matching plugin file: "
                    << entry.path().string() << std::endl;
            }
    ...
    if (plugin_path.empty()) {
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: uninstall_plugin - Could not find plugin file in "
                   "plugins directory"
                << std::endl;
        }
        return false;
    ...
    try {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: uninstall_plugin - Unloading plugin from memory"
                << std::endl;
        }
        unload_plugin(name);
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: uninstall_plugin - Successfully uninstalled plugin"
                << std::endl;
        }
        return true;
        ...
        std::cerr
            ...
            << e.what() << std::endl;
        if (g_debug_mode) {
            std::cerr << "DEBUG: uninstall_plugin - Filesystem error: "
                      << e.what() << std::endl;
        }
        return false;
    }
...
void Plugin::unload_plugin(const std::string& name) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: unload_plugin - Attempting to unload plugin: "
                  << name << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: unload_plugin - Plugins disabled, skipping unload"
                << std::endl;
        }
        return;
    }
    ...
    if (it != loaded_plugins.end()) {
        if (it->second.enabled && it->second.shutdown) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: unload_plugin - Calling shutdown function "
                             "for plugin: "
                          << name << std::endl;
            }
            it->second.shutdown();
        ...
        if (it->second.handle) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: unload_plugin - Closing library handle "
                             "for plugin: "
                          << name << std::endl;
            }
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: unload_plugin - Removing plugin from "
                         "loaded_plugins map"
                      << std::endl;
        }
        ...
        std::cerr
            << "DEBUG: unload_plugin - Plugin not found in loaded_plugins map"
            << std::endl;
    }
}
...
std::vector<std::string> Plugin::get_available_plugins() const {
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_available_plugins - Getting list of available "
                     "plugins"
                  << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_available_plugins - Plugins disabled, returning "
                   "empty list"
                << std::endl;
        }
        return {};
...
std::vector<std::string> Plugin::get_enabled_plugins() const {
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: get_enabled_plugins - Getting list of enabled plugins"
            << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_enabled_plugins - Plugins disabled, returning "
                   "empty list"
                << std::endl;
        }
        return {};
...
bool Plugin::enable_plugin(const std::string& name) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: enable_plugin - Attempting to enable plugin: "
                  << name << std::endl;
    }
    ...
    plugin_initialize_func init_func = nullptr;
    {
        ...
        if (lazy_loading_enabled && it == loaded_plugins.end()) {
            ...
            if (!load_plugin_on_demand(name)) {
                print_error({ErrorType::RUNTIME_ERROR,
                             ...
                             {}});
                if (g_debug_mode) {
                    std::cerr << "DEBUG: enable_plugin - Failed to load plugin "
                                 "on demand"
                              << std::endl;
                }
                return false;
        ...
        if (it == loaded_plugins.end()) {
            print_error({ErrorType::COMMAND_NOT_FOUND,
                         ...
                         {}});
            if (g_debug_mode) {
                std::cerr << "DEBUG: enable_plugin - Plugin not found in "
                             "loaded_plugins"
                          << std::endl;
            }
            return false;
    ...
    char** events = nullptr;
    {
        ...
        if (it == loaded_plugins.end()) {
            std::cerr << "Plugin not found after initialization: " << name
                      << std::endl;
            if (g_debug_mode) {
                std::cerr << "DEBUG: enable_plugin - Plugin not found after "
                             "initialization"
                          << std::endl;
            }
            return false;
        ...
        if (it->second.get_subscribed_events) {
            ...
            if (g_debug_mode) {
                std::cerr << "DEBUG: enable_plugin - Plugin subscribes to "
    ...
    if (events && count > 0) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: enable_plugin - Registering event subscriptions"
                << std::endl;
        }
        std::unique_lock events_lock(events_mutex);
        for (int i = 0; i < count; i++) {
    ...
    if (events && count > 0) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: enable_plugin - Freeing event memory"
                      << std::endl;
        }
        std::unique_lock plugins_lock(plugins_mutex);
        auto it = loaded_plugins.find(name);
...
bool Plugin::disable_plugin(const std::string& name) {
...

    {
        ...
        if (it != loaded_plugins.end() && it->second.enabled) {
            ...
            if (it->second.get_subscribed_events) {
                if (g_debug_mode) {
                    std::cerr << "DEBUG: disable_plugin - Getting subscribed "
                                 "events to "
                                 "unsubscribe"
                              << std::endl;
                }
            ...
            if (events && count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (g_debug_mode) {
                        std::cerr << "DEBUG: disable_plugin - Will unsubscribe "
                                     "from event: "
                                  << events[i] << std::endl;
                    }
                }
                if (it->second.free_memory) {
                    if (g_debug_mode) {
                        std::cerr
                            << "DEBUG: disable_plugin - Freeing event memory"
                            << std::endl;
                    }
                    for (int i = 0; i < count; i++) {
                        it->second.free_memory(events[i]);
            ...
            if (g_debug_mode) {
                std::cerr << "DEBUG: disable_plugin - Plugin not found or "
                             "already disabled"
                          << std::endl;
            }
            return false;
...

    {
        ...
        for (const auto& event : events_to_unsubscribe) {
            ...
            if (eventIt != subscribed_events.end()) {
                if (g_debug_mode) {
                    std::cerr << "DEBUG: disable_plugin - Removing plugin from "
                                 "event: "
                              << event << std::endl;
                }
                eventIt->second.erase(std::remove(eventIt->second.begin(),
                                                  eventIt->second.end(), name),
                                      eventIt->second.end());
            }
        }
    }
...
bool Plugin::handle_plugin_command(const std::string& targeted_plugin,
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: handle_plugin_command - Handling command for plugin: "
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: handle_plugin_command - Plugin system is disabled"
                << std::endl;
        }
        return false;
    }
    ...
    if (lazy_loading_enabled && it == loaded_plugins.end()) {
        ...
        if (load_plugin_on_demand(targeted_plugin)) {
            ...
            if (g_debug_mode) {
                std::cerr << "DEBUG: handle_plugin_command - Failed to load "
                             "plugin on "
                             "demand: "
    ...
    if (it != loaded_plugins.end() && it->second.enabled) {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: handle_plugin_command - Preparing arguments "
                         "for plugin"
                      << std::endl;
        }
...
std::vector<std::string> Plugin::get_plugin_commands(
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: get_plugin_commands - Getting commands for plugin: "
            << name << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_plugin_commands - Plugin system is disabled"
                << std::endl;
        }
        return {};
    }
    ...
    if (lazy_loading_enabled) {
        ...
        if (metadata_it != plugin_metadata_cache.end()) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: get_plugin_commands - Returning cached "
                             "commands, count: "
    ...
    if (it != loaded_plugins.end() && it->second.get_commands) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_plugin_commands - Calling plugin's get_commands "
                   "function"
                << std::endl;
        }
        int count = 0;
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: get_plugin_commands - Found " << count
                      << " commands" << std::endl;
        }
        ...
        if (it->second.free_memory) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: get_plugin_commands - Freeing command memory"
                    << std::endl;
            }
            for (int i = 0; i < count; i++) {
...
std::string Plugin::get_plugin_info(const std::string& name) const {
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_plugin_info - Getting info for plugin: "
                  << name << std::endl;
    }
...
bool Plugin::update_plugin_setting(const std::string& plugin_name,
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: update_plugin_setting - Updating setting for plugin: "
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: update_plugin_setting - Plugin system is disabled"
                << std::endl;
        }
        return false;
    }
    ...
    if (it != loaded_plugins.end()) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: update_plugin_setting - Found plugin, "
                         "updating setting"
                      << std::endl;
        }
        ...
        if (it->second.update_setting) {
            ...
            bool result = it->second.update_setting(
                              key.c_str(), value.c_str()) == PLUGIN_SUCCESS;
            if (g_debug_mode) {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: update_plugin_setting - Plugin not found"
                  << std::endl;
    }
    return false;
}
...
Plugin::get_all_plugin_settings() const {
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_all_plugin_settings - Getting settings for "
                     "all plugins"
                  << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_all_plugin_settings - Plugin system is disabled"
                << std::endl;
        }
        return {};
    }
...
void Plugin::trigger_subscribed_global_event(const std::string& event,
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: trigger_subscribed_global_event - Triggering event: "
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: trigger_subscribed_global_event - Plugin system is "
                   "disabled"
                << std::endl;
        }
        return;
...

    {
        ...
        if (it == subscribed_events.end() || it->second.empty()) {
            if (g_debug_mode) {
                std::cerr
                    << "DEBUG: trigger_subscribed_global_event - No plugins "
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: trigger_subscribed_global_event - Created event args: "
            << "args[0]=" << args.args[0] << ", "
            << "args[1]=" << args.args[1] << ", "
    ...
    for (const auto& plugin_name : subscribedPlugins) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: trigger_subscribed_global_event - Notifying plugin: "
        ...
        if (plugin_it != loaded_plugins.end() && plugin_it->second.enabled) {
            ...
            std::cerr
                << "DEBUG: trigger_subscribed_global_event - Plugin not found "
                   "or not enabled"
                << std::endl;
        }
    }
...
plugin_data* Plugin::get_plugin_data(const std::string& name) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_plugin_data - Getting data for plugin: "
                  << name << std::endl;
    }
...
void Plugin::clear_plugin_cache() {
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: clear_plugin_cache - Clearing plugin discovery cache"
            << std::endl;
    }
...

    {
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: clear_plugin_cache - Clearing subscribed events"
                << std::endl;
        }
        subscribed_events.clear();
    }
...
std::string Plugin::get_current_architecture() const {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_current_architecture - Raw architecture: "
                  << arch << std::endl;
    }
    ...
    if (arch == "x86_64" || arch == "amd64") {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_current_architecture - Normalized to: x86_64"
                << std::endl;
        }
        return "x86_64";
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_current_architecture - Normalized to: arm64"
                << std::endl;
        }
        return "arm64";
    }
...
std::string Plugin::get_file_architecture(
    ...
    if (!pipe) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: get_file_architecture - Failed to execute "
                         "file command"
                      << std::endl;
        }
        return result;
...
bool Plugin::is_architecture_compatible(const std::string& file_arch,
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: is_architecture_compatible - Checking compatibility "
               "between file: "
    ...
    if (file_arch == current_arch) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: is_architecture_compatible - Architectures "
                         "match exactly"
                      << std::endl;
        }
        return true;
    ...
    if (current_arch == "arm64" && file_arch == "x86_64") {
        ...
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: is_architecture_compatible - Apple ARM64 system "
                   "with x86_64 binary, "
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: is_architecture_compatible - Architectures are "
                     "incompatible"
                  << std::endl;
    }
    return false;
...
bool Plugin::is_rosetta_translated() const {
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: is_rosetta_translated - Checking if Rosetta is active"
            << std::endl;
    }
    ...
    if (sysctlbyname("sysctl.proc_translated", &ret, &size, NULL, 0) != -1) {
        ...
        std::cerr
            << "DEBUG: is_rosetta_translated - Failed to query Rosetta status"
            << std::endl;
    }
#else
    if (g_debug_mode) {
...
std::vector<std::string> Plugin::get_available_commands(
    ...
    if (g_debug_mode) {
        std::cerr
            << "DEBUG: get_available_commands - Getting commands for plugin: "
            << plugin_name << std::endl;
    }
    ...
    if (!enabled) {
        if (g_debug_mode) {
            std::cerr
                << "DEBUG: get_available_commands - Plugin system is disabled"
                << std::endl;
        }
        return {};
    }
