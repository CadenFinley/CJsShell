...
Plugin::Plugin(const std::filesystem::path& plugins_dir, bool enabled, bool lazy_loading) {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: Plugin constructor - Directory: " << plugins_dir << ", Enabled: " << (enabled ? "true" : "false")
                  << ", Lazy loading: " << (lazy_loading ? "true" : "false") << std::endl;
    }
    ...
    if (enabled) {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: Plugin constructor - Plugins are disabled" << std::endl;
        }
    }
}
...
Plugin::~Plugin() {
...

    {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: Plugin destructor - Clearing subscribed events" << std::endl;
        }
...
bool Plugin::discover_plugins() {
    ...
    if (g_debug_mode) {
        std::shared_lock plugins_lock(plugins_mutex);
        std::cerr << "DEBUG: discover_plugins - Discovery complete, loaded " << loaded_plugins.size() << " plugins" << std::endl;
    }
...
bool Plugin::load_plugin(const std::filesystem::path& path) {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: load_plugin - Plugin architecture: " << file_arch << ", System architecture: " << current_arch << std::endl;
    }
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: load_plugin - Plugin info retrieved - Name: " << info->name << ", Version: " << info->version
    ...
    if (g_debug_mode) {
        ...
        std::cerr << "DEBUG: load_plugin - initialize: " << (data.initialize ? "found" : "not found")
                  ...
                  << ", get_commands: " << (data.get_commands ? "found" : "not found")
                  << ", free_memory: " << (data.free_memory ? "found" : "not found") << std::endl;
    }
...
bool Plugin::extract_plugin_metadata(const std::filesystem::path& path, plugin_metadata& metadata) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: extract_plugin_metadata - Extracting metadata from: " << path.filename().string() << std::endl;
    }
    ...
    if (!temp_handle) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: extract_plugin_metadata - Failed to open library: " << dlerror() << std::endl;
        }
...
bool Plugin::load_plugin_on_demand(const std::string& name) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: load_plugin_on_demand - Loading plugin: " << name << std::endl;
    }

    {
        ...
        if (loaded_plugins.find(name) != loaded_plugins.end()) {
            if (g_debug_mode) {
                std::cerr << "DEBUG: load_plugin_on_demand - Plugin already loaded" << std::endl;
            }
            return true;
        }
    }
    ...
    if (metadata_it == plugin_metadata_cache.end()) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: load_plugin_on_demand - Plugin metadata not found" << std::endl;
        }
        return false;
    }
...
void Plugin::cache_plugin_metadata() {
    if (g_debug_mode) {
        std::cerr << "DEBUG: cache_plugin_metadata - Starting metadata caching" << std::endl;
    }

    if (!enabled) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: cache_plugin_metadata - Plugins disabled, skipping" << std::endl;
        }
        return;
    }
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: cache_plugin_metadata - Cached " << plugin_metadata_cache.size() << " plugin metadata entries" << std::endl;
    }
}
...
bool Plugin::handle_plugin_command(const std::string& targeted_plugin, std::vector<std::string>& args) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: handle_plugin_command - Handling command for plugin: " << targeted_plugin << ", args: ";
    ...
    if (it != loaded_plugins.end() && it->second.enabled) {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: handle_plugin_command - Result: " << (result == PLUGIN_SUCCESS ? "success" : "failure") << std::endl;
        }
...
bool Plugin::update_plugin_setting(const std::string& plugin_name, const std::string& key, const std::string& value) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: update_plugin_setting - Updating setting for plugin: " << plugin_name << ", key: " << key
    ...
    if (it != loaded_plugins.end()) {
        ...
        if (it->second.update_setting) {
            ...
            if (g_debug_mode) {
                std::cerr << "DEBUG: update_plugin_setting - Result: " << (result ? "success" : "failure") << std::endl;
            }
            return result;
        }
        return true;
...
std::map<std::string, std::map<std::string, std::string>> Plugin::get_all_plugin_settings() const {
    ...
    for (const auto& [name, data] : loaded_plugins) {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: get_all_plugin_settings - Found " << data.settings.size() << " settings for plugin: " << name << std::endl;
        }
    }
    return allSettings;
}
...
void Plugin::trigger_subscribed_global_event(const std::string& event, const std::string& event_data) {
    if (g_debug_mode) {
        std::cerr << "DEBUG: trigger_subscribed_global_event - Triggering event: " << event << " with data: " << event_data << std::endl;
    }
...

    {
        ...
        if (g_debug_mode) {
            std::cerr << "DEBUG: trigger_subscribed_global_event - Found " << subscribedPlugins.size() << " plugins subscribed to event"
                      << std::endl;
        }
    }
...
void Plugin::clear_plugin_cache() {
...

    {
        std::unique_lock events_lock(events_mutex);
        if (g_debug_mode) {
            std::cerr << "DEBUG: clear_plugin_cache - Clearing subscribed events" << std::endl;
        }
        subscribed_events.clear();
    }
...
bool Plugin::is_rosetta_translated() const {
    ...
    if (sysctlbyname("sysctl.proc_translated", &ret, &size, NULL, 0) != -1) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: is_rosetta_translated - Rosetta status: " << (ret == 1 ? "active" : "inactive") << std::endl;
        }
        return ret == 1;
    } else if (g_debug_mode) {
...
std::vector<std::string> Plugin::get_available_commands(const std::string& plugin_name) const {
    if (g_debug_mode) {
        std::cerr << "DEBUG: get_available_commands - Getting commands for plugin: " << plugin_name << std::endl;
    }

    if (!enabled) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: get_available_commands - Plugin system is disabled" << std::endl;
        }
        return {};
    }
