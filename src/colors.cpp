...
bool load_custom_colors(const std::string& filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if (std::regex_match(line, matches, color_def_regex)) {
      ...
      std::transform(color_name.begin(), color_name.end(), color_name.begin(),
                     ::toupper);
...
bool export_predefined_colors_to_file(const std::string& filename) {
  ...
  if (!file.is_open()) {
    std::cerr << "Error: Could not open file for writing: "
              << color_file_path.string() << std::endl;
    return false;
  }
  ...
  file << "# To create your own colors, you can edit this file or create a new "
...
bool ensure_default_colors_file_exists() {
  namespace fs = cjsh_filesystem::fs;
  fs::path default_colors_path =
      cjsh_filesystem::g_cjsh_colors_path / "default_colors.txt";
