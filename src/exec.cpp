...
void Exec::init_shell() {
  if (shell_is_interactive) {
    ...
    if (tcgetattr(shell_terminal, &shell_tmodes) < 0) {
      set_error(
          ErrorType::RUNTIME_ERROR, "tcgetattr",
...
int Exec::execute_command_async(const std::vector<std::string>& args) {
  ...
  if (pid == -1) {
    ...
    set_error(
        ErrorType::RUNTIME_ERROR, cmd_name,
    ...
    if (cmd.background) {
      ...
      if (!cmd.args.empty() && g_shell && g_shell->get_built_ins() &&
        ...
        if (cmd.args[0] == "__INTERNAL_SUBSHELL__") {
          ...
          try {
            if (!cmd.process_substitutions.empty()) {
              for (size_t i = 0; i < cmd.process_substitutions.size(); ++i) {
                ...
                if ((proc_sub.find("<(") == 0 || proc_sub.find(">(") == 0) &&
                    proc_sub.back() == ')') {
            ...
            if (!modified_cmd.output_file.empty()) {
              ...
              auto redirect_result =
                  cjsh_filesystem::FileOperations::redirect_fd(
                      modified_cmd.output_file.c_str(), STDOUT_FILENO,
            ...
            if (modified_cmd.both_output &&
              ...
              auto stdout_result = cjsh_filesystem::FileOperations::redirect_fd(
                  modified_cmd.both_output_file.c_str(), STDOUT_FILENO,
                  O_WRONLY | O_CREAT | O_TRUNC);
            ...
            if (!cmd.stderr_file.empty()) {
              ...
              if (redirect_result.is_error()) {
                throw std::runtime_error(
                    "cjsh: Failed to redirect stderr: " + cmd.stderr_file +
            ...
            if (!cmd.args.empty() && cmd.args[0] == "exec" &&
              ...
              exit_code =
                  g_shell->get_built_ins()->builtin_command(modified_cmd.args);
            }
          ...
          cjsh_filesystem::FileOperations::safe_dup2(orig_stdin, STDIN_FILENO);
      ...
      if (pid == 0) {
        pid_t child_pid = getpid();
        if (setpgid(child_pid, child_pid) < 0) {
          std::cerr << "cjsh: runtime error: setpgid: failed to set process "
                       "group ID in child: "
                    << strerror(errno) << std::endl;
          _exit(EXIT_FAILURE);
        }
        ...
        if (cmd.both_output && !cmd.both_output_file.empty()) {
          ...
          auto stderr_result = cjsh_filesystem::FileOperations::safe_dup2(
              STDOUT_FILENO, STDERR_FILENO);
          if (stderr_result.is_error()) {
            std::cerr << "cjsh: runtime error: dup2: failed for stderr in &> "
                         "redirection: "
        ...
        if (!cmd.append_file.empty()) {
          ...
          if (dup2(fd, STDOUT_FILENO) == -1) {
            std::cerr
                << "cjsh: runtime error: dup2: failed for append redirection: "
                << strerror(errno) << std::endl;
            close(fd);
            _exit(EXIT_FAILURE);
          }
        ...
        for (const auto& fd_dup : cmd.fd_duplications) {
          ...
          auto dup_result =
              cjsh_filesystem::FileOperations::safe_dup2(dst_fd, src_fd);
          if (dup_result.is_error()) {
            std::cerr << "cjsh: runtime error: dup2: failed for " << src_fd
                      << ">&" << dst_fd << ": " << dup_result.error()
                      << std::endl;
            _exit(EXIT_FAILURE);
          }
        }
    ...
    if (!job.completed) {
      if (killpg(job.pgid, 0) == 0) {
        if (killpg(job.pgid, SIGKILL) == 0) {
          ...
          if (errno != ESRCH) {
            set_error(
                ErrorType::RUNTIME_ERROR, "killpg",
                "failed to send SIGKILL in terminate_all_child_process: " +
                    std::string(strerror(errno)));
          }
        }
      }
