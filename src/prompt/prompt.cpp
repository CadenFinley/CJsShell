...
std::string Prompt::get_last_prompt_text() {
    if (!g_theme) {
        return "";
    }

    // Return the full prompt as it was last displayed
    std::string full_prompt = g_theme->get_last_ps1_prompt();
    if (g_theme->uses_newline()) {
        full_prompt += "\n";
        full_prompt += g_theme->get_last_newline_prompt();
    }
    return full_prompt;
}

std::string Prompt::get_last_prompt_ps1() {
    if (!g_theme) {
        return "";
    }
    return g_theme->get_last_ps1_prompt();
}

std::string Prompt::get_last_prompt_git() {
    if (!g_theme) {
        return "";
    }
    return g_theme->get_last_git_prompt();
}

std::string Prompt::get_last_prompt_newline() {
    if (!g_theme) {
        return "";
    }
    return g_theme->get_last_newline_prompt();
}

std::string Prompt::get_last_prompt_inline_right() {
    if (!g_theme) {
        return "";
    }
    return g_theme->get_last_inline_right_prompt();
}
