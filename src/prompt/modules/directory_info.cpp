...
std::string DirectoryInfo::get_display_directory() {
  ...
  // Apply repository contraction if enabled and in a git repo
...
std::string DirectoryInfo::substitute_path(const std::string& path,
...
std::string DirectoryInfo::truncate_path(const std::string& path, int max_length) {
  ...
  // Keep the last max_length components
...
std::string DirectoryInfo::to_fish_style(int dir_length, const std::string& full_path, 
  ...
  // Simplified fish-style implementation
  if (dir_length <= 0) {
