...
static int safe_execute_git_command(const std::string& command,
    ...
    if (pipe(pipefd) == -1) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: pipe() failed: " << strerror(errno)
                      << std::endl;
        }
        return -1;
    }
    ...
    if (pid == -1) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: fork() failed: " << strerror(errno)
                      << std::endl;
        }
    ...
    if (waitpid(pid, &status, 0) == -1) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: waitpid() failed: " << strerror(errno)
                      << std::endl;
        }
        return -1;
    }
...
std::string GitInfo::get_git_branch(
    ...
    if (g_debug_mode)
        std::cerr << "DEBUG: get_git_branch START for "
                  << git_head_path.string() << std::endl;
    ...
    try {
        ...
        if (head_contents.substr(0, ref_prefix.length()) == ref_prefix) {
            ...
            if (g_debug_mode)
                std::cerr << "DEBUG: get_git_branch END: " << branch
                          << std::endl;
            return branch;
        } else {
            if (g_debug_mode)
                std::cerr << "DEBUG: get_git_branch END: detached HEAD"
                          << std::endl;
...
std::string GitInfo::get_git_status(const std::filesystem::path& repo_root) {
    ...
    if (g_debug_mode)
        std::cerr << "DEBUG: get_git_status END: " << status_symbols
                  << std::endl;
