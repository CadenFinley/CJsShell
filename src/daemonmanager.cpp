...
std::vector<CronJob> DaemonManager::listCronJobs() {
    json command = {
        {"action", "cron"},
        {"cron_action", "list_jobs"}
    };
    ...
    try {
        json responseJson = json::parse(response);
        if (responseJson.contains("success") && responseJson["success"].get<bool>() && responseJson.contains("jobs")) {
            json jobsArray = responseJson["jobs"];
            for (const auto& jobJson : jobsArray) {
                jobs.push_back(CronJob::fromJson(jobJson));
            }
        }
    } catch (std::exception &e) {
    }
...
CronJob DaemonManager::getCronJob(const std::string& id) {
    json command = {
        {"action", "cron"},
        {"cron_action", "get_job"},
        {"id", id}
    ...
    try {
        json responseJson = json::parse(response);
        if (responseJson.contains("success") && responseJson["success"].get<bool>() && responseJson.contains("job")) {
            return CronJob::fromJson(responseJson["job"]);
        }
    } catch (std::exception &e) {
    }
    
    return CronJob();  // Return empty job if not found
}

std::string DaemonManager::addCronJob(const CronJob& job) {
    json command = {
        {"action", "cron"},
        {"cron_action", "add_job"},
        {"job", job.toJson()}
    ...
    try {
        json responseJson = json::parse(response);
        if (responseJson.contains("success") && responseJson["success"].get<bool>() && responseJson.contains("job_id")) {
            return responseJson["job_id"].get<std::string>();
        }
    } catch (std::exception &e) {
    }
...
bool DaemonManager::updateCronJob(const CronJob& job) {
    json command = {
        {"action", "cron"},
        {"cron_action", "update_job"},
        {"job", job.toJson()}
    };
...
bool DaemonManager::removeCronJob(const std::string& id) {
    json command = {
        {"action", "cron"},
        {"cron_action", "remove_job"},
        {"id", id}
    };
...
bool DaemonManager::enableCronJob(const std::string& id, bool enabled) {
    json command = {
        {"action", "cron"},
        {"cron_action", "enable_job"},
        {"id", id},
        {"enabled", enabled}
    };
...
// Cron Script Management Methods
std::vector<std::string> DaemonManager::listCronScripts() {
    json command = {
        {"action", "cron"},
        {"cron_action", "list_scripts"}
    };
    ...
    try {
        json responseJson = json::parse(response);
        if (responseJson.contains("success") && responseJson["success"].get<bool>() && responseJson.contains("scripts")) {
            scripts = responseJson["scripts"].get<std::vector<std::string>>();
        }
    } catch (std::exception &e) {
    }
...
std::string DaemonManager::getCronScript(const std::string& name) {
    json command = {
        {"action", "cron"},
        {"cron_action", "get_script"},
        {"name", name}
    };
    ...
    try {
        json responseJson = json::parse(response);
        if (responseJson.contains("success") && responseJson["success"].get<bool>() && responseJson.contains("content")) {
...
bool DaemonManager::saveCronScript(const std::string& name, const std::string& content) {
    json command = {
        {"action", "cron"},
        {"cron_action", "save_script"},
        {"name", name},
        {"content", content}
    };
...
bool DaemonManager::deleteCronScript(const std::string& name) {
    json command = {
        {"action", "cron"},
        {"cron_action", "delete_script"},
        {"name", name}
    };
