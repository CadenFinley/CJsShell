...
std::string CommandPreprocessor::process_here_documents(const std::string& command, 
...
std::string CommandPreprocessor::process_subshells(const std::string& command) {
    ...
    // Look for subshell pattern at start: (command) possibly followed by redirection
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: Converted subshell (" << subshell_content 
                  << ") to: sh -c '" << escaped_content << "'" << remaining << std::endl;
    }
