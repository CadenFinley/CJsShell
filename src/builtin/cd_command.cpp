...
int change_directory(const std::string& dir, std::string& current_directory,
  ...
  try {
    ...
    if (chdir(current_directory.c_str()) != 0) {
      ErrorInfo error = {
          ErrorType::RUNTIME_ERROR, "cd", std::string(strerror(errno)), {}};
      print_error(error);
...
int change_directory_smart(const std::string& dir,
  ...
  try {
    ...
    // Update bookmark database with the new directory, but only if we didn't
    // use a bookmark to get here and only if we used a relative path (to avoid
    // overriding bookmarks when navigating via relative paths)
