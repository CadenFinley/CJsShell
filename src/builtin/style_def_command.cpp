...
int style_def_command(const std::vector<std::string>& args) {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: Applied style for " << token_type << ": " << style << std::endl;
    }
...
void apply_custom_style(const std::string& token_type, const std::string& style) {
    ...
    if (g_debug_mode) {
        std::cerr << "DEBUG: Defined style " << full_style_name << " = " << style << std::endl;
    }
}
...
void load_custom_styles_from_config() {
    ...
    if (!config_file.is_open()) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: No .cjshrc file found, using default styles" << std::endl;
        }
        return;
    }
    ...
    while (std::getline(config_file, line)) {
        ...
        if (trimmed.find("style_def ") == 0) {
            ...
            if (!(iss >> token_type)) {
                if (g_debug_mode) {
                    std::cerr << "DEBUG: Invalid style_def at line " << line_number 
                              << ": missing token type" << std::endl;
                }
                continue;
            }
