...
int read_command(const std::vector<std::string>& args, Shell* shell) {
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (arg == "-r") {
            ...
            std::cout << "Usage: read [-r] [-p prompt] [-n nchars] [-d delim] [-t timeout] [name ...]\n";
            ...
            std::cout << "  -p prompt     output the string PROMPT without a trailing newline before reading\n";
            ...
            std::cout << "  -t timeout    time out and return failure if a complete line is not read within TIMEOUT seconds\n";
            return 0;
        } else if (arg[0] == '-') {
    ...
    // If no fields were read but we have input, it means the input was all whitespace
    // or the line was empty. In this case, set the first variable to the entire input.
