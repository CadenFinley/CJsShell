...
int times_command(const std::vector<std::string>& args, Shell* shell) {
    ...
    if (wall_time == (clock_t)-1) {
        print_error({ErrorType::RUNTIME_ERROR, "times", std::string("system call failed: ") + std::strerror(errno), {}});
        return 1;
    }

    long clock_ticks = sysconf(_SC_CLK_TCK);
    if (clock_ticks <= 0) {
        print_error({ErrorType::RUNTIME_ERROR, "times", "unable to get clock ticks per second", {}});
        return 1;
    }
    ...
    std::cout << format_time(user_time) << " " << format_time(system_time) << " " << format_time(child_user_time) << " "
