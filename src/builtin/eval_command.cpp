    ...
    if (eq_pos != std::string::npos && eq_pos > 0) {
      ...
      while (j < args.size()) {
        ...
        if (next_arg == ";" || next_arg == "&&" || next_arg == "||" || 
            next_arg == "|" || next_arg == "&" ||
            // Stop if next arg looks like a new command (doesn't contain path separators)
      ...
      if (full_value.find(" ") != std::string::npos) {
        bool already_quoted = (full_value.size() >= 2 && 
                              ((full_value.front() == '"' && full_value.back() == '"') ||
                               (full_value.front() == '\'' && full_value.back() == '\'')));
        
        if (!already_quoted) {
          // Remove any trailing semicolon from value before quoting
          bool has_semicolon = (!full_value.empty() && full_value.back() == ';');
          if (has_semicolon) {
            full_value.pop_back();
          }
