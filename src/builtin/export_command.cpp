...
int export_command(const std::vector<std::string>& args, Shell* shell) {
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (parse_env_assignment(args[i], name, value)) {
            ...
            if (g_debug_mode) {
                std::cout << "Set environment variable: " << name << "='"
                          << value << "'" << std::endl;
            }
        } else {
...
int unset_command(const std::vector<std::string>& args, Shell* shell) {
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (unsetenv(name.c_str()) != 0) {
            ...
            if (g_debug_mode) {
                std::cout << "Unset environment variable: " << name
                          << std::endl;
            }
        }
    }
...
int save_env_var_to_file(const std::string& name, const std::string& value,
    ...
    if (!login_mode) {
        if (g_debug_mode) {
            std::cerr
                << "Warning: Attempted to save environment variable to config "
...
int remove_env_var_from_file(const std::string& name, bool login_mode) {
    if (!login_mode) {
        if (g_debug_mode) {
            std::cerr
                << "Warning: Attempted to remove environment variable from "
