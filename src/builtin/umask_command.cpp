...
int umask_command(const std::vector<std::string>& args) {
  ...
  if (args.size() > 1 && args[1] == "-S") {
    if (args.size() == 2) {
      ...
      // User permissions
      std::cout << "u=";
      if (perms & S_IRUSR) std::cout << "r";
      if (perms & S_IWUSR) std::cout << "w";
      if (perms & S_IXUSR) std::cout << "x";
      
      std::cout << ",g=";
      if (perms & S_IRGRP) std::cout << "r";
      if (perms & S_IWGRP) std::cout << "w";
      if (perms & S_IXGRP) std::cout << "x";
      
      std::cout << ",o=";
      if (perms & S_IROTH) std::cout << "r";
      if (perms & S_IWOTH) std::cout << "w";
      if (perms & S_IXOTH) std::cout << "x";
  ...
  if (symbolic_mode || mode_str.find('=') != std::string::npos) {
    ...
    if (new_mask == current_mask && mode_str != "u=rwx,g=rwx,o=rwx") {
      std::cerr << "umask: " << mode_str << ": invalid symbolic mode" << std::endl;
      return 1;
    }
  } else {
    // Octal mode
    ...
    if (new_mask == static_cast<mode_t>(-1)) {
      std::cerr << "umask: " << mode_str << ": octal number out of range" << std::endl;
      return 1;
    }
  }
