...
int theme_command(const std::vector<std::string>& args) {
  ...
  if (args.size() < 2) {
    if (g_theme) {
      ...
      std::cout << "  theme preview <theme_name> - Preview a theme (local or remote) without loading it" << std::endl;
      std::cout << "  theme preview all         - Preview all available local themes" << std::endl;
      std::cout << "  theme install             - List themes available to install" << std::endl;
  ...
  if (args[1] == "preview") {
    if(g_theme) {
      ...
      if(theme_name == "all") {
        ...
        std::string theme_file = cjsh_filesystem::g_cjsh_theme_path.string() + "/" + theme_name + ".json";
        if (std::filesystem::exists(theme_file)) {
          return preview_theme(theme_name);
        } else {
          // If not local, try to preview from repository
          std::cout << "Theme '" << theme_name << "' not found locally. Trying to preview from repository..." << std::endl;
...
int list_available_themes() {
    ...
    std::string github_api_url = "https://api.github.com/repos/CadenFinley/CJsShell/contents/themes";
    ...
    // Parse JSON response
    std::vector<std::pair<std::string, std::string>> available_themes; // <name, download_url>
    try {
    ...
    if (available_themes.empty()) {
        std::cout << "No themes found or unable to parse repository contents." << std::endl;
        std::cout << "Please check your internet connection and try again." << std::endl;
        return 1;
    }
    ...
    try {
        ...
        std::cerr << "Warning: Could not read local themes directory: " << e.what() << std::endl;
    }
    
    // Print available themes
    std::cout << "Available themes to install:" << std::endl;
    for (const auto& [theme, url] : available_themes) {
        bool is_installed = std::find(installed_themes.begin(), installed_themes.end(), theme) != installed_themes.end();
        std::cout << "  " << theme;
        if (is_installed) {
            std::cout << " [installed]";
        }
    ...
    std::cout << "\nTo install a theme, use: theme install <theme_name>" << std::endl;
    std::cout << "To update an already installed theme, use the same command." << std::endl;
    std::cout << "For more information about a theme, use: theme info <theme_name>" << std::endl;
    return 0;
}

int preview_remote_theme(const std::string& themeName) {
    std::cout << "Fetching theme information for '" << themeName << "' from repository..." << std::endl;
    
    // GitHub raw content URL for the theme file
    std::string github_theme_url = "https://raw.githubusercontent.com/CadenFinley/CJsShell/master/themes/" + themeName + ".json";
    ...
    if (result.empty() || result.find("404: Not Found") != std::string::npos) {
        std::cerr << "Error: Theme '" << themeName << "' not found in repository." << std::endl;
        return 1;
    }
    ...
    try {
        ...
        if (theme_json.contains("ps1_segments") && theme_json["ps1_segments"].is_array()) {
            std::cout << "  Main Prompt Segments: " << theme_json["ps1_segments"].size() << std::endl;
        }
        
        if (theme_json.contains("git_segments") && theme_json["git_segments"].is_array()) {
            std::cout << "  Git Segments: " << theme_json["git_segments"].size() << std::endl;
        }
        
        if (theme_json.contains("ai_segments") && theme_json["ai_segments"].is_array()) {
            std::cout << "  AI Segments: " << theme_json["ai_segments"].size() << std::endl;
        }
        
        if (theme_json.contains("newline_segments") && theme_json["newline_segments"].is_array()) {
            std::cout << "  Newline Segments: " << theme_json["newline_segments"].size() << std::endl;
        }
        
        std::cout << "\nTo install this theme, use: theme install " << themeName << std::endl;
