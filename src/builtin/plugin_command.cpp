...
int plugin_command(const std::vector<std::string>& args) {
    ...
    if (cmd == "help") {
        ...
        std::cout << " commands [NAME]: List commands for a plugin"
                  << std::endl;
    ...
    if (cmd == "stats") {
        if (g_plugin) {
            ...
            std::cout << "Enabled plugins: "
                      << g_plugin->get_enabled_plugins().size() << std::endl;
            std::cout << "Loaded plugins: "
                      << g_plugin->get_loaded_plugin_count() << std::endl;
        ...
        if (g_plugin) {
            ...
            if (args.size() > 4 && args[3] == "set") {
                ...
                if (g_plugin->update_plugin_setting(pluginName, settingName,
                                                    settingValue)) {
                    std::cout << "Setting " << settingName << " set to "
                              << settingValue << " for plugin " << pluginName
                              << std::endl;
                } else {
                    std::cout << "Setting " << settingName
                              << " not found for plugin " << pluginName
                              << std::endl;
                }
                return 0;
            }
    ...
    if (g_plugin) {
        ...
        if (std::find(enabledPlugins.begin(), enabledPlugins.end(),
                      pluginName) != enabledPlugins.end()) {
            if (args.size() > 2) {
                if (args[2] == "enable") {
                    g_plugin->enable_plugin(pluginName);
                    return 0;
                ...
                if (args[2] == "info") {
                    std::cout << g_plugin->get_plugin_info(pluginName)
                              << std::endl;
                    return 0;
                }
                if (args[2] == "commands" || args[2] == "cmds" ||
                    args[2] == "help") {
                    std::cout << "Commands for " << pluginName << ":"
                              << std::endl;
                    std::vector<std::string> listOfPluginCommands =
                        g_plugin->get_plugin_commands(pluginName);
                    for (const auto& cmd : listOfPluginCommands) {
