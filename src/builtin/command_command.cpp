...
int command_command(const std::vector<std::string>& args, Shell* shell) {
    ...
    // Handle -v and -V options (describe the command)
    if (describe_command || verbose_description) {
        // Check if it's a builtin (command bypasses functions, but not builtins)
    ...
    // Save and optionally modify PATH
    std::string saved_path;
    if (use_default_path) {
        // NOLINTNEXTLINE(concurrency-mt-unsafe)
        const char* current_path = std::getenv("PATH");
    ...
    // Execute the command, bypassing functions
    // The shell's execute_command will check builtins first, then PATH
