...
int Built_ins::export_command(const std::vector<std::string>& args) {
  if (g_debug_mode) {
    std::cerr << "DEBUG: export_command called with " << args.size()
              << " arguments" << std::endl;
  }

  if (args.size() == 1) {
    if (g_debug_mode)
      std::cerr << "DEBUG: Listing all environment variables" << std::endl;
    extern char** environ;
...
int Built_ins::parse_assignment(const std::string& arg, std::string& name,
                                std::string& value) {
...
int Built_ins::save_alias_to_file(const std::string& name,
...
int Built_ins::save_env_var_to_file(const std::string& name,
                                    const std::string& value) {
  if (!shell || !shell->get_login_mode()) {
    if (g_debug_mode) {
