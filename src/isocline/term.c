...
typedef enum palette_e {
  ...
  ANSI16,   // basic + bright ANSI colors  (ESC[<idx>m, idx: 30-37, 90-97, +10
            // for background)
  ANSI256,  // ANSI 256 color palette      (ESC[38;5;<idx>m, idx: 0-15 standard
            // color, 16-231 6x6x6 rbg colors, 232-255 gray shades)
...
ic_private void term_set_attr(term_t* term, attr_t attr) {
  ...
  if (attr.x.color != term->attr.x.color && attr.x.color != IC_COLOR_NONE) {
    ...
    if (term->palette < ANSIRGB && color_is_rgb(attr.x.color)) {
      term->attr.x.color =
          attr.x.color;  // actual color may have been approximated but we keep
                         // the actual color to avoid updating every time
    }
  }
