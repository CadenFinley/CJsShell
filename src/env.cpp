...
void setup_path_variables(const struct passwd* pw) {
    ...
    if (cjsh_filesystem::file_exists("/usr/libexec/path_helper")) {
        if (g_debug_mode) {
            std::cerr << "DEBUG: Running /usr/libexec/path_helper via shell->execute" << std::endl;
        }
        ...
        if (g_shell) {
            ...
            std::cerr << "DEBUG: Shell not available for path_helper execution" << std::endl;
        }
    }
    #endif
    ...
    if (path_env && path_env[0] != '\0') {
        ...
        if (getenv("MANPATH") == nullptr) {
            ...
            for (const auto& path : manpaths) {
                if (cjsh_filesystem::file_exists(path)) {
                    if (!manpath_str.empty()) manpath_str += ":";
                    manpath_str += path;
                }
            }
...
std::vector<std::pair<const char*, const char*>> setup_user_system_vars(const struct passwd* pw) {
    ...
    // Set environment variables directly instead of storing in vector with dangling pointers
