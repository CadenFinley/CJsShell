...
std::vector<std::string> generate_command_suggestions(const std::string& command) {
    ...
    std::unordered_set<std::string> all_commands_set; // Use set to avoid duplicates
    ...
    static const std::vector<std::string> shell_builtins = {
        ...
        "eval", "syntax", "history", "exit", "quit", "terminal", "prompt_test", 
        "test", "[", "exec", "trap", "jobs", "fg", "bg", "wait", "kill", "readonly", 
    ...
    std::vector<std::string> common_commands = {
        ...
        "head", "tail", "less", "more", "cat", "tac", "rev", "tr", "paste",
        "join", "comm", "diff", "cmp", "file", "stat", "touch", "cp", "mv",
        "rm", "rmdir", "mkdir", "ln", "chmod", "chown", "chgrp", "umask",
        "tar", "gzip", "gunzip", "zip", "unzip", "bzip2", "bunzip2", "xz", "unxz",
        "curl", "wget", "ping", "ssh", "scp", "rsync", "ftp", "sftp",
        "ps", "top", "htop", "kill", "killall", "jobs", "bg", "fg", "nohup",
        "df", "du", "mount", "umount", "fdisk", "free", "uptime", "uname",
        "whoami", "who", "w", "last", "finger", "id", "groups", "su", "sudo",
        ...
        "vim", "vi", "nano", "emacs", "less", "more", "cat"
    };
...
std::vector<std::string> generate_fuzzy_suggestions(const std::string& command, 
    ...
    // Special handling for single letter commands - only suggest commands that start with that letter
    if (command.length() == 1) {
    ...
    for (const auto& cmd : available_commands) {
        ...
        if (cmd == command || seen_commands.count(cmd)) continue;
...
int calculate_fuzzy_score(const std::string& input, const std::string& candidate) {
    if (input.empty() || candidate.empty()) return 0;
    
    // Exact match gets highest score
    if (input == candidate) return 1000;
    ...
    for (char c : input) input_chars[std::tolower(c)]++;
    for (char c : candidate) candidate_chars[std::tolower(c)]++;
