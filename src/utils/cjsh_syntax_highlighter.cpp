...
bool SyntaxHighlighter::is_function_definition(const std::string& input,
                                               size_t& func_name_start,
    ...
    // Pattern 1: "function name" followed by optional { or nothing (still
    // typing)
    if (trimmed.substr(first_non_space, 8) == "function") {
        ...
        if (name_start >= trimmed.length()) {
            return false;  // Just "function" with no name
        }
    ...
    // Pattern 2: "name()" followed by optional { or nothing (still typing)
    ...
    if (paren_pos != std::string::npos && paren_pos >= first_non_space) {
        ...
        // Skip any whitespace before the parentheses
        while (name_end > name_start && std::isspace(trimmed[name_end - 1])) {
            name_end--;
        }

        if (name_end > name_start) {
            // Check if the name contains only valid identifier characters
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        if (c == '\'' && !in_double_quote) {
            if (!in_single_quote) {
                ...
                if (quote_end < length) {
                    quote_end++;
                    ic_highlight(henv, start + quote_start,
                                 quote_end - quote_start, "cjsh-string");
                    i = quote_end - 1;
...
void SyntaxHighlighter::highlight(ic_highlight_env_t* henv, const char* input,
    ...
    // Check for function definitions first (both in menu mode and AI mode)
    ...
    if (is_function_definition(input_str, func_name_start, func_name_end)) {
        // Highlight the function name with the function definition style
        ic_highlight(henv, func_name_start, func_name_end - func_name_start,
                     "cjsh-function-definition");

        // Also highlight the parentheses if present
        ...
        // Highlight opening brace if present
    ...
    while (pos < len) {
        ...
        if (!token.empty()) {
            if (token.rfind("./", 0) == 0 || token.rfind("../", 0) == 0 ||
                ...
                if (token.rfind("~/", 0) == 0) {
                    path_to_check = cjsh_filesystem::g_user_home_path.string() +
                                    token.substr(1);
        ...
        while (arg_start < cmd_str.length()) {
            ...
            if (is_redirection_operator(arg)) {
                ...
                if (is_quoted_string(arg, quote_type)) {
                    ic_highlight(henv, cmd_start + arg_start,
                                 arg_end - arg_start, "cjsh-string");
                    ...
                    if (arg.rfind("./", 0) == 0) {
                        if (!std::filesystem::exists(arg) ||
                            ...
                            ic_highlight(henv, cmd_start + arg_start,
                                         arg_end - arg_start, "cjsh-installed");
                        }
                    } else {
                        auto cmds = g_shell->get_available_commands();
                        if (std::find(cmds.begin(), cmds.end(), arg) !=
                            cmds.end()) {
                            ic_highlight(henv, cmd_start + arg_start,
                                         arg_end - arg_start, "cjsh-builtin");
                        } else if (is_shell_builtin(arg)) {
                            ic_highlight(henv, cmd_start + arg_start,
                                         arg_end - arg_start, "cjsh-builtin");
                        } else if (basic_unix_commands_.count(arg) > 0) {
                            ic_highlight(henv, cmd_start + arg_start,
                                         arg_end - arg_start, "cjsh-system");
                    ...
                    ic_highlight(henv, cmd_start + arg_start,
                                 arg_end - arg_start, "cjsh-path-exists");
                } else if (is_glob_pattern(arg)) {
                    ic_highlight(henv, cmd_start + arg_start,
                                 arg_end - arg_start, "cjsh-glob-pattern");
                    ...
                    if (arg.rfind("~/", 0) == 0) {
                        ...
                        std::string prev_dir =
                            g_shell->get_previous_directory();
                        if (!prev_dir.empty()) {
            ...
            if (!is_variable_reference(arg) &&
                arg.find('$') != std::string::npos) {
                highlight_quotes_and_variables(
                    henv, input, cmd_start + arg_start, arg_end - arg_start);
            }
