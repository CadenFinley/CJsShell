        ...
        if (dir_it != added_completions.end()) {
          if (g_debug_mode) {
            std::cerr << "DEBUG: Skipping bookmark '" << completion_text
                      << "' because directory with same name exists"
                      << std::endl;
          }
          return true;
        }
          ...
          if (bookmark_it != added_completions.end() &&
            ...
            if (g_debug_mode) {
              std::cerr << "DEBUG: Directory '" << completion_text
                        << "' will replace existing bookmark with same name"
                        << std::endl;
            }
  ...
  bool add_completion_prim_if_unique(const char* completion_text,
    ...
    if (g_debug_mode)
      std::cerr << "DEBUG: Adding unique completion (prim): '"
                ...
                << "' (total: " << total_completions_added << ")" << std::endl;
    return ic_add_completion_prim_with_source(cenv, completion_text, display,
                                              help, source, delete_before,
                                              delete_after);
  }
  ...
  bool add_completion_prim_with_source_if_unique(
    ...
    if (has_reached_completion_limit()) {
      if (g_debug_mode)
        std::cerr << "DEBUG: Reached global completion limit ("
                  ...
                  << "), skipping (prim with source): '" << completion_text
                  << "'" << std::endl;
      return true;
    }
    ...
    if (it == added_completions.end()) {
      ...
      if (new_priority <= existing_priority) {
        if (g_debug_mode)
          std::cerr << "DEBUG: Skipping lower/equal priority completion (prim "
                       "with source): '"
      ...
      if (g_debug_mode)
        std::cerr << "DEBUG: Replacing lower priority completion with: '"
                  ...
                  << (source ? source : "null")
                  << "', priority: " << new_priority
                  << " vs existing: " << existing_priority << ")" << std::endl;
    }
...
bool safe_add_completion_prim_with_source(ic_completion_env_t* cenv,
  ...
  if (g_current_completion_tracker) {
    return g_current_completion_tracker
        ->add_completion_prim_with_source_if_unique(completion_text, display,
                                                    help, source, delete_before,
                                                    delete_after);
  } else {
    return ic_add_completion_prim_with_source(cenv, completion_text, display,
...
void cjsh_history_completer(ic_completion_env_t* cenv, const char* prefix) {
  ...
  while (std::getline(history_file, line) && matches.size() < 50) {
    ...
    if (looks_like_file_path(line)) {
      if (g_debug_mode)
        std::cerr << "DEBUG: Skipping path-like history entry: '" << line << "'"
                  << std::endl;
      continue;
    }
  ...
  for (const auto& match : matches) {
    ...
    if (!safe_add_completion_prim_with_source(cenv, completion.c_str(), nullptr,
                                              nullptr, "history", delete_before,
                                              0))
      return;
  ...
  if (!prefix_before.empty()) {
    ...
    if (command_part == "cd" || command_part.rfind("cd ", 0) == 0) {
      if (!config::smart_cd_enabled) {
        if (g_debug_mode)
          std::cerr
              << "DEBUG: Skipping bookmark completions - smart CD is disabled"
              << std::endl;
      } else {
        if (g_debug_mode)
                ...
                if (fs::exists(current_dir_item) &&
                  ...
                  if (g_debug_mode)
                    std::cerr << "DEBUG: Skipping bookmark '" << bookmark_name
                              << "' because a directory with the same name "
  ...
  switch (context) {
    ...
    case CONTEXT_ARGUMENT: {
      ...
      if (!tokens.empty() && tokens[0] == "cd") {
        if (g_debug_mode)
          std::cerr
              << "DEBUG: Detected cd command, using only filename completion"
              << std::endl;
        cjsh_filename_completer(cenv, prefix);
      } else {
        cjsh_history_completer(cenv, prefix);
