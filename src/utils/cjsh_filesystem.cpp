...
bool should_refresh_executable_cache() {
    try {
        // Check if PATH environment has changed
        if (has_path_changed()) {
            if (g_debug_mode) {
...
void create_profile_file() {
    std::string profile_content =
        ...
        "# cjshopt login-startup-arg --interactive         # Force interactive mode\n"
        "# cjshopt login-startup-arg --debug               # Enable debug mode\n"
        "# cjshopt login-startup-arg --minimal             # Disable all unique cjsh "
        ...
        "# cjshopt login-startup-arg --no-ai               # Disable AI features\n"
        "# cjshopt login-startup-arg --no-colors           # Disable colors\n"
        "# cjshopt login-startup-arg --no-titleline        # Disable title line\n"
        "# cjshopt login-startup-arg --show-startup-time   # Display shell startup time\n"
        "# cjshopt login-startup-arg --no-source           # Don't source the .cjshrc "
        "file\n"
        "# cjshopt login-startup-arg --no-completions      # Disable tab completions\n"
        "# cjshopt login-startup-arg --no-syntax-highlighting # Disable syntax "
        "highlighting\n"
        "# cjshopt login-startup-arg --no-smart-cd         # Disable smart cd "
        "functionality\n"
        "# cjshopt login-startup-arg --disable-custom-ls   # Use system ls command "
        "instead of builtin ls\n"
        "# cjshopt login-startup-arg --startup-test        # Enable startup test "
        "mode\n";
...
bool init_interactive_filesystem() {
    ...
    try {
        ...
        if (should_refresh_cache) {
            ...
            // Even if we don't rebuild the full cache, occasionally clean up
            // stale entries This helps remove executables that were uninstalled
            // since last full refresh
            static int cleanup_counter = 0;
            if (++cleanup_counter % 10 == 0) {  // Every 10th shell startup
                if (g_debug_mode)
                    std::cerr
...
void add_executable_to_cache(const std::string& executable_name,
    ...
    // First check if it's already in cache to avoid duplicate writes
    ...
    // Add the new executable (just the name, like the original cache)
    ...
    // Notify other systems of cache update
    if (write_result.is_ok()) {
...
std::string get_current_path_hash() {
    ...
    // Create a hash of the PATH environment variable
...
bool has_path_changed() {
    ...
    if (current_hash.empty()) {
        return true;  // If we can't get current hash, assume it changed
    }
    ...
    if (read_result.is_error()) {
        // No cached hash means PATH has "changed" (first run)
...
void remove_executable_from_cache(const std::string& executable_name) {
    ...
    // Only rewrite cache if something was actually removed
...
void cleanup_stale_cache_entries() {
    ...
    // Only rewrite cache if we removed anything
    if (removed_count > 0) {
        std::string content;
...
void notify_cache_systems_of_update() {
    ...
    // Refresh syntax highlighter's executable cache
    SyntaxHighlighter::refresh_executables_cache();

    // Refresh completions system (though it already reads fresh data each time)
