...
BookmarkDatabase::BookmarkDatabase() : dirty_(false) {
    ...
    database_path_ = (cjsh_filesystem::g_cjsh_cache_path / "directory_bookmarks.json").string();
}
...
cjsh_filesystem::Result<void> BookmarkDatabase::save() {
    ...
    // Write to file
    auto write_result = cjsh_filesystem::FileOperations::write_file_content(database_path_, json_content);
...
cjsh_filesystem::Result<void> BookmarkDatabase::from_json(const std::string& json_str) {
    try {
        using json = nlohmann::json;
        ...
        if (!root.contains("version")) {
            return cjsh_filesystem::Result<void>::error("Invalid bookmark database: missing version");
        }
        
        std::string version = root["version"];
        if (version != "1.0") {
            return cjsh_filesystem::Result<void>::error("Unsupported bookmark database version: " + version);
        }
        ...
        if (root.contains("bookmarks") && root["bookmarks"].is_object()) {
            for (const auto& [name, bookmark_data] : root["bookmarks"].items()) {
                ...
                if (bookmark_data.contains("path") && bookmark_data["path"].is_string()) {
                    entry.path = bookmark_data["path"];
                } else {
                    continue;  // Skip entries without valid path
                }
                
                if (bookmark_data.contains("access_count") && bookmark_data["access_count"].is_number()) {
                    entry.access_count = bookmark_data["access_count"];
                } else {
                    entry.access_count = 1;
                }
                
                if (bookmark_data.contains("added_time") && bookmark_data["added_time"].is_string()) {
                    entry.added_time = time_from_iso_string(bookmark_data["added_time"]);
                } else {
                    entry.added_time = std::chrono::system_clock::now();
                }
                
                if (bookmark_data.contains("last_accessed") && bookmark_data["last_accessed"].is_string()) {
                    entry.last_accessed = time_from_iso_string(bookmark_data["last_accessed"]);
                } else {
        ...
        return cjsh_filesystem::Result<void>::error("Failed to parse bookmark database JSON: " + std::string(e.what()));
    } catch (const std::exception& e) {
        return cjsh_filesystem::Result<void>::error("Error loading bookmark database: " + std::string(e.what()));
    }
}
