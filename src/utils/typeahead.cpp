...
std::string capture_available_input() {
  ...
  if ((fd_flags & O_NONBLOCK) == 0) {
    if (fcntl(STDIN_FILENO, F_SETFL, fd_flags | O_NONBLOCK) == 0) {
      restore_flags = true;
      if (g_debug_mode) {
        std::cerr << "DEBUG: Typeahead capture temporarily enabled O_NONBLOCK"
                  << std::endl;
      }
  ...
  if (tcgetattr(STDIN_FILENO, &original_termios) == 0) {
    ...
    if (tcsetattr(STDIN_FILENO, TCSANOW, &raw_termios) == 0) {
      ...
      if (g_debug_mode) {
        std::cerr << "DEBUG: Typeahead capture temporarily disabled ICANON/ECHO"
                  << std::endl;
      }
    } else if (g_debug_mode) {
  ...
  struct RestoreState {
    ...
    ~RestoreState() {
      ...
      if (restore_flags) {
        ...
        if (g_debug_mode) {
          std::cerr << "DEBUG: Typeahead capture restored O_NONBLOCK flag"
                    << std::endl;
        }
      }
    }
  ...
  if (captured_data.empty()) {
    ...
    if (g_debug_mode) {
      std::cerr << "DEBUG: Typeahead poll after read returned " << poll_result
                << " (revents=0x" << std::hex << pfd.revents << std::dec << ")"
                << std::endl;
    }
  }
