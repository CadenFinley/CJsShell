...
public:
    ...
    void initializeDefaults() {
        ...
        // Set initial default values to prevent empty or "N/A" values on first render
    ...
    std::string fetchDataForKey(const std::string& key) {
        ...
        if (key == "CPU_USAGE") {
            FILE* fp = popen("top -l 1 | grep 'CPU usage' | awk '{print $3}' | tr -d '%' || echo '0'", "r");
            if (!fp) return "0";
            ...
            FILE* fp = popen("ps -A -o %mem | awk '{ sum += $1 } END { print sum }' || echo '0'", "r");
            if (!fp) return "0";
            ...
            FILE* fp = popen("ipconfig getifaddr en0 2>/dev/null || ipconfig getifaddr en1 2>/dev/null || echo 'N/A'", "r");
            if (!fp) return "N/A";
            ...
            FILE* fp = popen("route -n get default 2>/dev/null | grep interface | awk '{print $2}' || echo 'N/A'", "r");
            if (!fp) return "N/A";
