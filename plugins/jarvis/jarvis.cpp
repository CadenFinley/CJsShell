...
extern "C" PLUGIN_API int plugin_initialize() {
  ...
  if (access(script_path.c_str(), F_OK) != 0) {
    ...
    fprintf(script_file, "    os.environ[\"VOSK_LOG_LEVEL\"] = \"0\"  # 0 = no logs, 1 = errors, 2 = warnings, 3 = info\n");
    fprintf(script_file, "    os.environ[\"KALDI_LOG_LEVEL\"] = \"0\"  # Completely silence Kaldi logs\n");
    fprintf(script_file, "    \n");
    ...
    fprintf(script_file, "    \n");
    fprintf(script_file, "    # Redirect stderr temporarily during model loading to suppress logs\n");
    fprintf(script_file, "    original_stderr = sys.stderr\n");
    fprintf(script_file, "    sys.stderr = open(os.devnull, 'w')\n");
    fprintf(script_file, "    \n");
    fprintf(script_file, "    model_path = os.path.expanduser(\"~/.config/cjsh/Jarvis/vosk-model-small-en-us-0.15\")\n");
    ...
    fprintf(script_file, "        #print(\"[system] Ready. Say 'jarvis' to wake me up.\", file=sys.stderr)\n");
    ...
    fprintf(script_file, "                        # Select a random response when Jarvis is activated\n");
    fprintf(script_file, "                        response = random.choice(jarvis_responses)\n");
    fprintf(script_file, "                        print(f\"\\n[jarvis] {response}\", file=sys.stderr)\n");
    fprintf(script_file, "                        active = True\n");
    fprintf(script_file, "                        last_active = time.time()\n");
    fprintf(script_file, "                        continue\n\n");
    fprintf(script_file, "                    if active:\n");
    fprintf(script_file, "                        # Only print the actual command to stdout (no prefixes)\n");
    fprintf(script_file, "                        # This will be treated as a command to execute\n");
    fprintf(script_file, "                        print(text)  # forward command\n");
    fprintf(script_file, "                        sys.stdout.flush()\n");
    fprintf(script_file, "                        last_active = time.time()  # reset timeout\n");
    ...
    fprintf(script_file, "                        # optional: print partials for real-time feedback\n");
    fprintf(script_file, "                        # print(f\"(partial) {text}\", file=sys.stderr)\n");
    fprintf(script_file, "                        last_active = time.time()\n\n");
    fprintf(script_file, "            # Timeout handling\n");
    fprintf(script_file, "            if active and (time.time() - last_active > ACTIVE_TIMEOUT):\n");
    fprintf(script_file, "                active = False\n");
    fprintf(script_file, "                #print(\"[hotword] timeout, listening again\", file=sys.stderr)\n\n");
    ...
    if (access(model_dir.c_str(), F_OK) != 0) {
      ...
      std::cout << "[jarvis] Please download the Vosk model and extract it to this location" << std::endl;
    }
  }
