...
class PromptInfoCache {
   private:
    ...
    std::vector<std::string> core_keys = {"CPU_USAGE", "MEM_USAGE",  "BATTERY",
                                          "TIME",      "DATE",       "IP_LOCAL",
                                          "NET_IFACE", "GIT_STATUS", "GIT_BRANCH"};

    // How frequently to refresh data in seconds
    std::unordered_map<std::string, int> refresh_intervals = {
        {"CPU_USAGE", 5},  {"MEM_USAGE", 5},  {"BATTERY", 30},    {"TIME", 1},
   ...
   public:
    ...
    std::string fetchDataForKey(const std::string& key) {
        ...
        if (key == "CPU_USAGE") {
            ...
            FILE* fp = popen(
                "ps -A -o %mem | awk '{ sum += $1 } END { print sum }' || echo "
                "'0'",
                "r");
            if (!fp)
                return "0";
            ...
            FILE* fp = popen(
                "route -n get default 2>/dev/null | grep interface | awk "
                "'{print "
                "$2}' || echo 'N/A'",
                "r");
            if (!fp)
            ...
            FILE* fp = popen(
                "git rev-parse --is-inside-work-tree 2>/dev/null || echo "
                "'false'",
                "r");
            if (!fp)
                return "N/A";
            ...
            if (key == "GIT_STATUS") {
                ...
                FILE* fp = popen(
                    "git symbolic-ref --short HEAD 2>/dev/null || git "
                    "rev-parse "
                ...
                FILE* fp = popen(
                    "git stash list 2>/dev/null | wc -l | tr -d ' ' || echo "
                    "'0'",
                    "r");
                if (!fp)
                    return "0";
    ...
    std::string getValue(const std::string& key) {
        ...
        if (it != cache.end()) {
            // Return the expired value while a refresh is happening in the
            // background
            return it->second.value;
        }
...
extern "C" PLUGIN_API plugin_info_t* plugin_get_info() {
    static plugin_info_t info = {(char*)"fast_prompt_tags", (char*)"0.3.0",
...
extern "C" PLUGIN_API plugin_validation_t plugin_validate() {
    ...
    // Note: This is called before initialization, so we can't check runtime
    // state We can only validate static/compile-time things here

    // Check if we can access the PromptInfoCache class (basic sanity check)
    try {
        // Just verify we can instantiate the class - don't keep the instance
        PromptInfoCache* test_cache = new PromptInfoCache();
        delete test_cache;
    } catch (...) {
...
extern "C" PLUGIN_API char** plugin_get_commands(int* count) {
    ...
    *count = 0;
    return (char**)PLUGIN_MALLOC(0);  // Return empty heap-allocated array
}
