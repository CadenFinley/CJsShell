...
class SpotifyStatusPlugin : public PluginInterface {
 private:
  ...
  std::string formatStatusLine(const std::string& title,
  ...
  bool refreshAccessToken() {
    ...
    std::string auth_header =
        "Basic " + base64Encode(CLIENT_ID + std::string(":") + CLIENT_SECRET);
    ...
    if (res != CURLE_OK) {
      std::cerr << "Token refresh failed: " << curl_easy_strerror(res)
                << std::endl;
      return false;
    }
    ...
    try {
      ...
      tokenExpiry = std::chrono::system_clock::now() +
  ...
  json getCurrentPlayback() {
    ...
    struct curl_slist* headers = nullptr;
    headers = curl_slist_append(
        headers, ("Authorization: Bearer " + accessToken).c_str());

    std::string response;
    curl_easy_setopt(curl, CURLOPT_URL,
                     (std::string(SPOTIFY_API_URL) + "/me/player").c_str());
    ...
    while (running) {
      try {
        ...
        if (!accessToken.empty()) {
          ...
          auto timeSinceLastSuccess =
              std::chrono::duration_cast<std::chrono::seconds>(
                  now - lastSuccessfulConnection)
                  .count();

          // If it's been significantly longer than our update interval, system
          // might have been asleep
          if (lastSuccessfulConnection.time_since_epoch().count() > 0 &&
              timeSinceLastSuccess > updateInterval * 3) {
            std::cerr << "Potential system sleep detected, reconnecting..."
                      << std::endl;
            needsReconnection = true;
          ...
          if (!playbackState.empty()) {
...

            {
              std::lock_guard<std::mutex> lock(mutex);
              currentStatus =
                  formatStatusLine(title, artist, timeInfo, isPlaying);
            }
  ...
  void saveUserData() {
    std::filesystem::path userDataPath =
        std::filesystem::path(getPluginDirectory()) / "spotify";
  ...
  void loadUserData() {
    std::filesystem::path userDataPath =
        std::filesystem::path(getPluginDirectory()) / "spotify/user-data.json";
    ...
    if (std::filesystem::exists(userDataPath)) {
      ...
      if (file.is_open()) {
        try {
          ...
          std::cerr << "Error loading Spotify user data: " << e.what()
                    << std::endl;
        }
      }
    }
  ...
  bool startAuthServer() {
    ...
    int opt = 1;
    if (setsockopt(callbackServerSocket, SOL_SOCKET, SO_REUSEADDR, &opt,
                   sizeof(opt)) < 0) {
    ...
    // Bind
    if (bind(callbackServerSocket, (struct sockaddr*)&server, sizeof(server)) <
        0) {
    ...
    serverThread =
        std::thread(&SpotifyStatusPlugin::handleAuthServerConnections, this);
    return true;
  }
  ...
  void handleAuthServerConnections() {
    ...
    while (running && !authCodeReceived) {
      // Accept connection
      int client_sock = accept(callbackServerSocket, (struct sockaddr*)&client,
                               (socklen_t*)&c);
      if (client_sock < 0) {
        continue;
      }
      ...
      if (codePos != std::string::npos) {
        ...
        if (codeEnd != std::string::npos) {
          ...
          std::string response =
              ...
              "align-items: center; "
              "min-height: 100vh; background: linear-gradient(135deg, #1DB954 "
              "0%, #1ed760 100%); color: white; }"
              ".container { background: rgba(0,0,0,0.2); border-radius: 12px; "
              "padding: 2rem; "
              "text-align: center; backdrop-filter: blur(10px); max-width: "
              "500px; margin: 1rem; }"
              "h1 { margin-bottom: 1rem; }"
              ".logo { width: 64px; height: 64px; margin-bottom: 1rem; }"
              ".checkmark { font-size: 64px; margin-bottom: 1rem; animation: "
              "pop 0.5s ease-out; }"
              "@keyframes pop { 0% { transform: scale(0); } 100% { transform: "
              "scale(1); } }"
              "p { line-height: 1.5; opacity: 0.9; }"
              "</style></head>"
              "<body><div class='container'>"
              "<img class='logo' "
              "src='https://storage.googleapis.com/pr-newsroom-wp/1/2018/11/"
        ...
        std::string response =
            ...
            "<p>If you haven't authorized the app yet, you should be "
  ...
  void stopAuthServer() {
    if (callbackServerSocket >= 0) {
      ::shutdown(callbackServerSocket,
                 SHUT_RDWR);  // interrupt blocking accept()
      close(callbackServerSocket);
  ...
  void startAuthFlow() {
    ...
    // Note: This uses the application's CLIENT_ID but will authenticate the
    // user's personal Spotify account Each user who runs this command will
    // connect their own Spotify account
...
// Open URL in default browser
    ...
    std::cout << "Please authorize the application in your browser."
              << std::endl;
    ...
    if (authCodeReceived) {
      ...
      if (exchangeCodeForToken(capturedAuthCode)) {
        ...
        std::cout << "✗ Failed to exchange authorization code for tokens."
                  << std::endl;
        std::cout << "This could be due to an expired authorization code or "
                     "network issues."
                  << std::endl;
        std::cout << "Try running 'spotify auth' again to get a fresh "
                     "authorization code."
                  << std::endl;
      }
    } else {
      std::cout << "\n✗ Timed out waiting for authorization." << std::endl;
      std::cout << "If you authorized the app, you can try manually using:"
                << std::endl;
    ...
    try {
      ...
      if (j.contains("error")) {
        ...
        if (j.contains("error_description")) {
          std::cerr << "Error description: " << j["error_description"]
                    << std::endl;
        }
        return false;
      }
 ...
 public:
  ...
  std::string getName() const override { return "SpotifyStatus"; }
  ...
  bool initialize() override {
    ...
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL,
                     1L);  // Prevent SIGALRM during DNS timeout
    ...
    running = true;
    update_thread =
        std::thread(&SpotifyStatusPlugin::updateSpotifyStatus, this);
    return true;
  }
  ...
  std::vector<std::string> getCommands() const override { return {"spotify"}; }

  std::vector<std::string> getSubscribedEvents() const override { return {}; }

  int getInterfaceVersion() const override { return 2; }
  ...
  bool handleCommand(std::queue<std::string>& args) override {
    ...
    if (command == "spotify") {
      if (args.empty()) {
        ...
        std::cout << "  spotify interval [SECONDS] - Set update interval"
                  << std::endl;
        std::cout << "  spotify show - Show status in terminal title"
                  << std::endl;
        std::cout << "  spotify hide - Hide status from terminal title"
                  << std::endl;
        std::cout << "  spotify enable - Enable the plugin" << std::endl;
        std::cout << "  spotify disable - Disable the plugin" << std::endl;
        std::cout << "  spotify auth - Start Spotify authorization flow "
                     "(required first step)"
                  << std::endl;
        std::cout << "  spotify logout - Clear authentication data"
                  << std::endl;
        return true;
      }
      ...
      if (subcommand == "status") {
        std::lock_guard<std::mutex> lock(mutex);
        std::cout << "Current Spotify status: "
                  << (currentStatus.empty() ? "Not playing" : currentStatus)
                  << std::endl;
        std::cout << "Update interval: " << updateInterval << " seconds"
                  << std::endl;
        std::cout << "Status visible: " << (visible ? "Yes" : "No")
                  << std::endl;
        std::cout << "Plugin enabled: " << (disabled ? "No" : "Yes")
                  << std::endl;
        std::cout << "Authenticated: " << (!refreshToken.empty() ? "Yes" : "No")
                  << std::endl;
        if (refreshToken.empty()) {
          std::cout << "\nTo use this plugin, you need to authenticate with "
                       "Spotify first:"
                    << std::endl;
          std::cout << "  Run 'spotify auth' to start the authorization process"
                    << std::endl;
        }
        return true;
        ...
        if (setting == "refresh_token" && !args.empty()) {
          ...
          if (!refreshAccessToken()) {
            std::cout << "Warning: Failed to validate refresh token"
                      << std::endl;
          }
          saveUserData();
          return true;
        ...
        try {
          ...
          args.pop();
          std::cout << "Update interval set to: " << updateInterval
                    << " seconds" << std::endl;
          saveUserData();
          return true;
        } catch (const std::exception& e) {
        ...
        visible = true;
        displayStatus();
        std::cout << "Spotify status is now visible in terminal title"
                  << std::endl;
        saveUserData();
        return true;
        ...
        visible = false;
        resetTerminalTitle();
        std::cout << "Spotify status is now hidden from terminal title"
                  << std::endl;
        saveUserData();
        return true;
        ...
        if (args.empty()) {
          ...
          std::cout << "This will connect YOUR Spotify account to this plugin."
                    << std::endl;
          std::cout << "Note: Each user can authorize their own Spotify "
                       "account with this plugin."
                    << std::endl;
          startAuthFlow();
          return true;
        } else {
          ...
          if (exchangeCodeForToken(code)) {
            std::cout << "Successfully authenticated with Spotify!"
                      << std::endl;
          } else {
        ...
        refreshToken = "";
        accessToken = "";
        std::cout << "Spotify authentication data cleared. Use 'spotify auth' "
                     "to reconnect."
                  << std::endl;
        saveUserData();
        return true;
  ...
  std::map<std::string, std::string> getDefaultSettings() const override {
    return {{"update_interval", std::to_string(updateInterval)},
