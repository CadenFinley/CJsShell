...
// clang++ -dynamiclib -fPIC -O2 -std=c++11 -o ClaudeAnthropic.dylib
// ClaudeAnthropic.cpp -lcurl
...
class ClaudeAnthropic : public PluginInterface {
 private:
  ...
  void writeUserDataFile() {
    ...
    if (file.is_open()) {
      ...
      std::cerr << "Error: Unable to write to the user data file at "
                << USER_DATA << std::endl;
    }
  }
  ...
  void readUserDataFile() {
    ...
    if (file.is_open()) {
      ...
      std::cerr << "Error: Unable to read the user data file at " << USER_DATA
                << std::endl;
    }
  }
 ...
 public:
  ClaudeAnthropic() : curl(nullptr), maxTokens(1000) {
    ...
    USER_DATA = (std::filesystem::path(SETTINGS_DIRECTORY) /
                 "claude-anthropic-settings.json")
                    .string();
  }
  ...
  std::string getName() const { return "Claude Anthropic"; }
    ...
    if (!std::filesystem::exists(SETTINGS_DIRECTORY)) {
      ...
      if (!checkFile.good()) {
        ...
        if (file.is_open()) {
          ...
          std::cerr << "Error: Unable to create the user data file at "
                    << USER_DATA << std::endl;
        }
      }
    }
  ...
  bool handleCommand(std::queue<std::string>& args) {
    ...
    if (cmd == "chat") {
      ...
      std::string payload =
          "{\"model\":\"claude-3-7-sonnet-20250219\",\"max_tokens\":" +
        ...
        if (res != CURLE_OK) {
          ...
          try {
            ...
            if (content.is_array() && !content.empty()) {
              std::cout << "Claude: " << content[0]["text"].get<std::string>()
                        << std::endl;
            } else {
