...
class CustomCoutBuffer : public std::streambuf {  // Removed extra dot
 ...
 protected:
  ...
  std::string applySyntaxHighlighting(const std::string& line,
    ...
    // Generic number regex that matches integers, decimals, scientific
    // notation, and hex
    ...
    if (lang == "python" || lang == "py") {
      ...
      std::vector<std::string> keywords = {
          "def",    "class",   "elif",   "import",   "from",   "as",    "try",
          "except", "finally", "with",   "in",       "is",     "and",   "or",
          "not",    "pass",    "lambda", "nonlocal", "global", "async", "await",
          "yield",  "assert",  "del",    "raise"};
      keywords.insert(keywords.end(), commonKeywords.begin(),
                      commonKeywords.end());

      // Python numbers
      result =
          std::regex_replace(result, numberRegex,
                             syntaxColors.number + "$&" + syntaxColors.normal);

      // Python strings
      std::regex stringRegex("(['\"])(.*?)\\1");
      result =
          std::regex_replace(result, stringRegex,
                             syntaxColors.string + "$&" + syntaxColors.normal);

      // Python comments
      std::regex commentRegex("#.*$");
      result =
          std::regex_replace(result, commentRegex,
      ...
      for (const auto& keyword : keywords) {
        ...
        result = std::regex_replace(
            result, keywordRegex,
            syntaxColors.keyword + keyword + syntaxColors.normal);
      }
    } else if (lang == "cpp" || lang == "c++" || lang == "c") {
      // C++ keywords
      std::vector<std::string> keywords = {
          "auto",    "case",     "class",     "const",     "default",
          "do",      "enum",     "extern",    "goto",      "register",
          "sizeof",  "static",   "struct",    "switch",    "typedef",
          "union",   "volatile", "new",       "delete",    "try",
          "catch",   "throw",    "namespace", "using",     "template",
          "virtual", "friend",   "public",    "private",   "protected",
          "inline",  "explicit", "typename",  "constexpr", "override",
          "final",   "nullptr",  "noexcept",  "decltype",  "mutable",
          "operator"};
      keywords.insert(keywords.end(), commonKeywords.begin(),
                      commonKeywords.end());
      ...
      std::vector<std::string> types = {
          ...
          "short", "long",   "unsigned", "signed", "size_t", "wchar_t",
          "auto",  "string", "vector",   "map"};
      ...
      result =
          std::regex_replace(result, lineCommentRegex,
                             syntaxColors.comment + "$&" + syntaxColors.normal);
      ...
      for (const auto& type : types) {
        ...
        result = std::regex_replace(
            result, typeRegex,
            syntaxColors.keyword + type + syntaxColors.normal);
      }
      ...
      for (const auto& value : values) {
        ...
        result = std::regex_replace(
            result, valueRegex,
            syntaxColors.number + value + syntaxColors.normal);
      }
    } else if (lang == "java") {
      // Java keywords
      std::vector<std::string> keywords = {
          "abstract", "assert",    "boolean",    "byte",      "case",
          "catch",    "char",      "class",      "const",     "default",
          "do",       "double",    "enum",       "extends",   "final",
          "finally",  "float",     "implements", "import",    "instanceof",
      ...
      std::vector<std::string> keywords = {
          ...
          "new",    "this",   "super",   "delete",   "in",     "of",
          "do",     "case",   "switch",  "default",  "void",   "with",
          "yield"};
      keywords.insert(keywords.end(), commonKeywords.begin(),
                      commonKeywords.end());
      ...
      if (lang == "ts" || lang == "typescript") {
        std::vector<std::string> tsKeywords = {
            "interface",  "namespace", "module",    "enum",     "type",
            "implements", "any",       "string",    "number",   "boolean",
            "public",     "private",   "protected", "readonly", "abstract",
            "declare",    "keyof",     "never",     "unknown"};
      ...
      while (
          std::regex_search(searchStart, result.cend(), tagMatch, tagRegex)) {
      ...
      // HTML/XML attributes - use regex_search instead of regex_replace with
      // lambda
      ...
      while (
          std::regex_search(searchStart, result.cend(), attrMatch, attrRegex)) {
        // Add the text before the match
        ...
        processed.append(syntaxColors.function + attrMatch[1].str() +
                         syntaxColors.normal);
      ...
      // HTML/XML attribute values - this one works fine with standard
      // regex_replace
      ...
      result =
          std::regex_replace(result, importantRegex,
                             syntaxColors.keyword + "$1" + syntaxColors.normal);
      ...
      result =
          std::regex_replace(result, atRuleRegex,
                             syntaxColors.keyword + "$1" + syntaxColors.normal);

      // CSS units
      std::regex unitRegex("(\\d+)(px|em|rem|vh|vw|%)");
      result =
          std::regex_replace(result, unitRegex,
                             syntaxColors.number + "$1" + syntaxColors.keyword +
                                 "$2" + syntaxColors.normal);

      // CSS colors
      std::regex colorRegex("#[a-fA-F0-9]{3,6}");
      result = std::regex_replace(
          result, colorRegex, syntaxColors.number + "$&" + syntaxColors.normal);
    } else if (lang == "go" || lang == "golang") {
      // Go keywords
      std::vector<std::string> keywords = {
          "package",   "import", "func",        "type",  "struct",
          "interface", "map",    "chan",        "const", "var",
          "go",        "defer",  "select",      "case",  "default",
          "switch",    "range",  "fallthrough", "goto"};
      keywords.insert(keywords.end(), commonKeywords.begin(),
                      commonKeywords.end());
      ...
      // Rust values
      std::vector<std::string> values = {"true", "false", "None",
                                         "Some", "Ok",    "Err"};
      ...
      std::regex charRegex("'.'");
      result = std::regex_replace(
          result, charRegex, syntaxColors.string + "$&" + syntaxColors.normal);
      ...
      for (const auto& macro : macros) {
        ...
        result = std::regex_replace(
            result, macroRegex,
            syntaxColors.function + macro + syntaxColors.normal + "!");
      }
      ...
      std::vector<std::string> keywords = {
          ...
          "INDEX",    "INTO",   "VALUES", "AND",         "OR",       "NOT",
          "JOIN",     "LEFT",   "RIGHT",  "OUTER",       "INNER",    "FULL",
          "GROUP",    "BY",     "HAVING", "ORDER",       "ASC",      "DESC",
          "LIMIT",    "OFFSET", "SET",    "TRANSACTION", "COMMIT",   "ROLLBACK",
          "GRANT",    "REVOKE", "ON",     "TO",          "WITH",     "AS",
      ...
      // PHP keywords
      std::vector<std::string> keywords = {
          "abstract",  "and",          "array",      "as",         "catch",
          "class",     "clone",        "const",      "declare",    "default",
      ...
      result = std::regex_replace(
          result, varRegex, syntaxColors.number + "$&" + syntaxColors.normal);
      ...
      result =
          std::regex_replace(result, symbolRegex,
                             syntaxColors.number + "$&" + syntaxColors.normal);
      ...
      std::vector<std::string> keywords = {
          ...
          "in",     "function", "time",   "select", "break",    "continue",
          "return", "exit",     "export", "local",  "readonly", "shift",
    ...
    // Added new language support with number highlighting
      ...
      std::vector<std::string> keywords = {
          ...
          "for",  "while",    "return", "class",    "struct",
          "enum", "protocol", "import", "extension"};
      std::regex stringRegex("\"(\\\\.|[^\"])*\"");
      result =
          std::regex_replace(result, stringRegex,
                             syntaxColors.string + "$&" + syntaxColors.normal);
      ...
      result =
          std::regex_replace(result, blockCommentRegex,
                             syntaxColors.comment + "$&" + syntaxColors.normal);
      std::regex funcRegex("\\bfunc\\s+([a-zA-Z_][a-zA-Z0-9_]*)");
      result = std::regex_replace(
          result, funcRegex,
          syntaxColors.keyword + "func" + syntaxColors.normal + " " +
              syntaxColors.function + "$1" + syntaxColors.normal);
      for (const auto& keyword : keywords) {
        std::regex keywordRegex("\\b" + keyword + "\\b");
        result = std::regex_replace(
      ...
      std::vector<std::string> keywords = {
          "fun",    "val",       "var", "if",    "else",   "when",   "class",
          "object", "interface", "for", "while", "return", "import", "package"};
      std::regex stringRegex("\"(\\\\.|[^\"])*\"");
      result =
          std::regex_replace(result, stringRegex,
                             syntaxColors.string + "$&" + syntaxColors.normal);
      ...
      result = std::regex_replace(
          result, funcRegex,
          syntaxColors.keyword + "fun" + syntaxColors.normal + " " +
              syntaxColors.function + "$1" + syntaxColors.normal);
      for (const auto& keyword : keywords) {
        std::regex keywordRegex("\\b" + keyword + "\\b");
        result = std::regex_replace(
      ...
      std::vector<std::string> keywords = {"module", "import", "where", "do",
                                           "let",    "in",     "if",    "then",
                                           "else",   "case",   "of"};
      std::regex stringRegex("\"(\\\\.|[^\"])*\"");
      result =
      ...
      result = std::regex_replace(
          result, funcRegex,
          syntaxColors.keyword + "function" + syntaxColors.normal + " " +
              syntaxColors.function + "$1" + syntaxColors.normal);
      for (const auto& keyword : keywords) {
        std::regex keywordRegex("\\b" + keyword + "\\b");
        result = std::regex_replace(
      ...
      std::vector<std::string> keywords = {
          "def",     "val",  "var",    "if",     "else",   "match",
          "case",    "for",  "yield",  "class",  "object", "trait",
          "extends", "with", "import", "package"};
      std::regex stringRegex("\"(\\\\.|[^\"])*\"");
      result =
          std::regex_replace(result, stringRegex,
                             syntaxColors.string + "$&" + syntaxColors.normal);
      ...
      result = std::regex_replace(
          result, funcRegex,
          syntaxColors.keyword + "def" + syntaxColors.normal + " " +
              syntaxColors.function + "$1" + syntaxColors.normal);
      for (const auto& keyword : keywords) {
        std::regex keywordRegex("\\b" + keyword + "\\b");
        result = std::regex_replace(
 ...
 public:
  CustomCoutBuffer(std::streambuf* orig) : original(orig), enabled(true) {}
...
class SyntaxHighlighter : public PluginInterface {
 ...
 public:
  SyntaxHighlighter() : originalCoutBuffer(nullptr), customBuffer(nullptr) {}
  ...
  std::string getDescription() const {
    return "Applies syntax highlighting to code blocks in std::cout output";
  }
  ...
  bool initialize() {
    ...
    std::cout.rdbuf(customBuffer);  // Removed static_cast as it's already
  ...
  bool handleCommand(std::queue<std::string>& args) {
    ...
    if (cmd == "coutoverride" && !args.empty()) {
      ...
      if (subCmd == "highlight" && !args.empty()) {
        ...
        std::cout << "Code highlighting " << (enabled ? "enabled" : "disabled")
                  << std::endl;
        return true;
      }
    }
  ...
  int getInterfaceVersion() const { return 2; }
