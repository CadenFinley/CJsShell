...
class PasswordPlugin : public PluginInterface {
 private:
  ...
  // New helper method to read password input with prompt and then erase the
  // prompt line
  ...
  void ensurePasswordDirectoryExists() {
    std::filesystem::path pwDir =
        std::filesystem::path(getPluginsHomeDirectory()) / "pw";
  ...
  bool handlePasswordPrompt() {
    if (passwordHash.empty()) {
      ...
      while (attempts < maxAttempts) {
        ...
        std::string inputPassword = readAndErasePasswordInput(
            "(" + std::to_string(remainingAttempts) + ") Enter password: ");
 ...
 public:
  ...
  virtual std::string getName() const override { return "PasswordProtection"; }
  ...
  virtual std::string getAuthor() const override { return "Caden Finley"; }
  ...
  virtual void shutdown() override { enabled = false; }
  ...
  virtual bool handleCommand(std::queue<std::string>& args) override {
    ...
    if (command == "event" && !args.empty()) {
      ...
      if (action == "set") {
        if (!passwordHash.empty()) {
          std::cout
              << "Password is already set. Use 'password change' to update it."
              << std::endl;
        } else {
          std::string newPassword =
              readAndErasePasswordInput("Enter new password: ");
        ...
        if (!passwordHash.empty()) {
          std::string currentPassword =
              readAndErasePasswordInput("Enter current password: ");
          ...
          std::cout << "No password is currently set. Use 'password set' first."
                    << std::endl;
        }
        return true;
      } else if (action == "remove") {
        if (!passwordHash.empty()) {
          std::string currentPassword = readAndErasePasswordInput(
              "Enter current password to remove password protection: ");
          if (verifyPassword(currentPassword)) {
            passwordHash = "";
        ...
        std::cout << "Password file location: " << passwordFilePath
                  << std::endl;
        return true;
      }
    }
  ...
  virtual int getInterfaceVersion() const override { return 2; }

  virtual std::map<std::string, std::string> getDefaultSettings()
      const override {
