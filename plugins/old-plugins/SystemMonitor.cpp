...
class SystemMonitor : public PluginInterface {
 private:
  ...
  double getCPUUsage() {
    ...
    vm_deallocate(mach_task_self_, (vm_address_t)cpuLoad,
  ...
  std::pair<double, double> getMemoryUsage() {
    ...
    host_statistics64(machPort, HOST_VM_INFO64, (host_info64_t)&vmStats,
                      &count);

    double used = (vmStats.active_count + vmStats.wire_count) * pageSize;
    double total = (vmStats.active_count + vmStats.wire_count +
                    vmStats.inactive_count + vmStats.free_count) *
                   pageSize;

    return {used / (1024.0 * 1024.0),
            total / (1024.0 * 1024.0)};  // Return in MB
  }
  ...
  std::pair<double, double> getDiskUsage() {
    ...
    return {used / (1024.0 * 1024.0 * 1024.0),
            total / (1024.0 * 1024.0 * 1024.0)};  // Return in GB
  }
 ...
 public:
  ...
  ~SystemMonitor() override { shutdown(); }
  ...
  std::string getDescription() const override {
    return "System resource monitor for macOS with DevToolsTerminal usage "
           "tracking";
  }
  ...
  void shutdown() override { monitoring = false; }
 ...
 private:
  void displaySystemInfo() {
    ...
    std::cout << "Memory Usage: " << (memUsage.first / memUsage.second) * 100
              << "% "
              << "(Used: " << memUsage.first
              << "MB / Total: " << memUsage.second << "MB)\n";
    std::cout << "Disk Usage: " << (diskUsage.first / diskUsage.second) * 100
              << "% "
              << "(Used: " << diskUsage.first
              << "GB / Total: " << diskUsage.second << "GB)\n";
  }
  ...
  void displayProgramInfo() {
    ...
    std::cout << "Commands/Minute: " << std::fixed << std::setprecision(2)
              << (runtime.count() > 0
                      ? static_cast<double>(commandsProcessed) / runtime.count()
                      : 0)
              << "\n";
  }
