...
PLUGIN_API int plugin_initialize() {
    char* plugin_dir = plugin_get_plugin_directory("ColorError");
    if (plugin_dir) {
        SETTINGS_DIRECTORY = std::string(plugin_dir);
        plugin_free_string(plugin_dir);
        USER_DATA = (fs::path(SETTINGS_DIRECTORY) / "color-error-settings.json").string();
...
PLUGIN_API int plugin_handle_command(plugin_args_t* args) {
    ...
    if (cmd == "setcolor" && args->count > 1) {
        ...
        if (color.find_first_not_of("0123456789;") == std::string::npos) {
            plugin_update_setting("color", color.c_str());
            return PLUGIN_SUCCESS;
        }
