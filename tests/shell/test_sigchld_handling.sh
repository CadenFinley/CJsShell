...
# Helper function to check for zombies
check_for_zombies() {
    local timeout=${1:-5}
    local count=0
...
if [ "$RESULT" = "result" ] && check_for_zombies 3; then
...
if check_for_zombies 5; then
    echo "PASS: final zombie check - no zombies remaining"
else
    echo "FAIL: final zombie check - zombies still present"
