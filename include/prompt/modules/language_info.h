...
class LanguageInfo {
 private:
  ...
  std::vector<std::string> python_files = {"requirements.txt", "requirements-dev.txt", "pyproject.toml", 
                                           "Pipfile", "Pipfile.lock", "setup.py", "setup.cfg", "tox.ini",
                                           ".python-version", "environment.yml", "conda.yml"};
  std::vector<std::string> python_extensions = {".py", ".pyi", ".pyx"};
  std::vector<std::string> python_folders = {"__pycache__", ".venv", "venv", "env"};

  std::vector<std::string> nodejs_files = {"package.json", "package-lock.json", "yarn.lock", 
                                           ".node-version", ".nvmrc", "pnpm-lock.yaml"};
  std::vector<std::string> nodejs_extensions = {".js", ".mjs", ".cjs", ".ts", ".tsx", ".vue"};
  ...
  std::vector<std::string> golang_files = {"go.mod", "go.sum", "Gopkg.toml", "Gopkg.lock", 
                                           "glide.yaml", "glide.lock", "Godeps"};
  std::vector<std::string> golang_extensions = {".go"};

  std::vector<std::string> java_files = {"pom.xml", "build.gradle", "build.gradle.kts", 
                                         "build.xml", "gradle.properties"};
