#pragma once

#include <filesystem>
#include <string>
#include <unordered_map>
#include <vector>

#include "basic_info.h"
#include "command_info.h"
#include "container_info.h"
#include "directory_info.h"
#include "environment_info.h"
#include "git_info.h"
#include "language_info.h"
#include "network_info.h"
#include "system_info.h"
#include "time_info.h"

struct ThemeSegment;

class PromptInfo {
   public:
    std::string get_basic_prompt();
    std::string get_basic_title();
    bool is_variable_used(const std::string& var_name, const std::vector<ThemeSegment>& segments);
    std::unordered_map<std::string, std::string> get_variables(
        const std::vector<ThemeSegment>& segments, bool is_git_repo = false,
        const std::filesystem::path& repo_root = {});

    // bool is_root_path(const std::filesystem::path& path) {
    //     return is_root_path(path);
    // }
    // std::string get_current_file_name() {
    //     return get_current_file_name();
    // }
    // std::string get_current_file_path() {
    //     return get_current_file_path();
    // }
    // std::string get_username() {
    //     return get_username();
    // }
    // std::string get_hostname() {
    //     return get_hostname();
    // }

    // std::string get_git_branch(const std::filesystem::path& git_head_path) {
    //     return get_git_branch(git_head_path);
    // }
    // std::string get_git_status(const std::filesystem::path& repo_root) {
    //     return get_git_status(repo_root);
    // }
    // std::string get_local_path(const std::filesystem::path& repo_root) {
    //     return get_local_path(repo_root);
    // }
    // std::string get_git_remote(const std::filesystem::path& repo_root) {
    //     return get_git_remote(repo_root);
    // }
    // std::string get_git_tag(const std::filesystem::path& repo_root) {
    //     return get_git_tag(repo_root);
    // }
    // std::string get_git_last_commit(const std::filesystem::path& repo_root) {
    //     return get_git_last_commit(repo_root);
    // }
    // std::string get_git_author(const std::filesystem::path& repo_root) {
    //     return get_git_author(repo_root);
    // }
    // int get_git_ahead_behind(const std::filesystem::path& repo_root, int& ahead, int& behind) {
    //     return get_git_ahead_behind(repo_root, ahead, behind);
    // }
    // int get_git_stash_count(const std::filesystem::path& repo_root) {
    //     return get_git_stash_count(repo_root);
    // }
    // bool get_git_has_staged_changes(const std::filesystem::path& repo_root) {
    //     return get_git_has_staged_changes(repo_root);
    // }
    // int get_git_uncommitted_changes(const std::filesystem::path& repo_root) {
    //     return get_git_uncommitted_changes(repo_root);
    // }

    // std::string get_os_info() {
    //     return get_os_info();
    // }
    // std::string get_kernel_version() {
    //     return get_kernel_version();
    // }
    // float get_cpu_usage() {
    //     return get_cpu_usage();
    // }
    // float get_memory_usage() {
    //     return get_memory_usage();
    // }
    // std::string get_battery_status() {
    //     return get_battery_status();
    // }
    // std::string get_uptime() {
    //     return get_uptime();
    // }
    // std::string get_disk_usage(const std::filesystem::path& path = "/") {
    //     return get_disk_usage(path);
    // }
    // std::string get_swap_usage() {
    //     return get_swap_usage();
    // }
    // std::string get_load_avg() {
    //     return get_load_avg();
    // }

    // std::string get_terminal_type() {
    //     return get_terminal_type();
    // }
    // std::pair<int, int> get_terminal_dimensions() {
    //     return get_terminal_dimensions();
    // }
    // std::string get_active_language_version(const std::string& language) {
    //     return get_active_language_version(language);
    // }
    // bool is_in_virtual_environment(std::string& env_name) {
    //     return is_in_virtual_environment(env_name);
    // }
    // int get_background_jobs_count() {
    //     return get_background_jobs_count();
    // }
    // std::string get_shell() {
    //     return get_shell();
    // }
    // std::string get_shell_version() {
    //     return get_shell_version();
    // }

    // std::string get_ip_address(bool external = false) {
    //     return get_ip_address(external);
    // }
    // bool is_vpn_active() {
    //     return is_vpn_active();
    // }
    // std::string get_active_network_interface() {
    //     return get_active_network_interface();
    // }

    // std::string get_current_time(bool twelve_hour_format = false) {
    //     return get_current_time(twelve_hour_format);
    // }
    // std::string get_current_date() {
    //     return get_current_date();
    // }
    // int get_current_day() {
    //     return get_current_day();
    // }
    // int get_current_month() {
    //     return get_current_month();
    // }
    // int get_current_year() {
    //     return get_current_year();
    // }
    // std::string get_current_day_name() {
    //     return get_current_day_name();
    // }
    // std::string get_current_month_name() {
    //     return get_current_month_name();
    // }

    // std::string get_display_directory() {
    //     return get_display_directory();
    // }
    // std::string get_directory_name() {
    //     return get_directory_name();
    // }
    // std::string get_truncated_path() {
    //     return get_truncated_path();
    // }
    // std::string get_repo_relative_path(const std::filesystem::path& repo_root) {
    //     return get_repo_relative_path(repo_root);
    // }
    // bool is_directory_truncated() {
    //     return is_truncated();
    // }

    // void start_command_timing() {
    //     start_command_timing();
    // }
    // void end_command_timing(int exit_code) {
    //     end_command_timing(exit_code);
    // }
    // void reset_command_timing() {
    //     reset_command_timing();
    // }
    // void set_initial_duration(long long microseconds) {
    //     set_initial_duration(microseconds);
    // }
    // long long get_last_command_duration_us() {
    //     return get_last_command_duration_us();
    // }
    // std::string get_formatted_duration() {
    //     return get_formatted_duration();
    // }
    // bool should_show_duration() {
    //     return should_show_duration();
    // }
    // int get_last_exit_code() {
    //     return get_last_exit_code();
    // }
    // std::string get_exit_status_symbol() {
    //     return get_exit_status_symbol();
    // }
    // bool is_last_command_success() {
    //     return is_last_command_success();
    // }

    // bool is_python_project() {
    //     return is_python_project();
    // }
    // bool is_nodejs_project() {
    //     return is_nodejs_project();
    // }
    // bool is_rust_project() {
    //     return is_rust_project();
    // }
    // bool is_golang_project() {
    //     return is_golang_project();
    // }
    // bool is_java_project() {
    //     return is_java_project();
    // }
    // bool is_cpp_project() {
    //     return is_cpp_project();
    // }
    // bool is_csharp_project() {
    //     return is_csharp_project();
    // }
    // bool is_php_project() {
    //     return is_php_project();
    // }
    // bool is_ruby_project() {
    //     return is_ruby_project();
    // }
    // bool is_kotlin_project() {
    //     return is_kotlin_project();
    // }
    // bool is_swift_project() {
    //     return is_swift_project();
    // }
    // bool is_dart_project() {
    //     return is_dart_project();
    // }
    // bool is_scala_project() {
    //     return is_scala_project();
    // }
    // std::string get_python_version() {
    //     return get_python_version();
    // }
    // std::string get_nodejs_version() {
    //     return get_nodejs_version();
    // }
    // std::string get_rust_version() {
    //     return get_rust_version();
    // }
    // std::string get_golang_version() {
    //     return get_golang_version();
    // }
    // std::string get_java_version() {
    //     return get_java_version();
    // }
    // std::string get_cpp_version() {
    //     return get_cpp_version();
    // }
    // std::string get_csharp_version() {
    //     return get_csharp_version();
    // }
    // std::string get_php_version() {
    //     return get_php_version();
    // }
    // std::string get_ruby_version() {
    //     return get_ruby_version();
    // }
    // std::string get_kotlin_version() {
    //     return get_kotlin_version();
    // }
    // std::string get_swift_version() {
    //     return get_swift_version();
    // }
    // std::string get_dart_version() {
    //     return get_dart_version();
    // }
    // std::string get_scala_version() {
    //     return get_scala_version();
    // }
    // std::string get_python_virtual_env() {
    //     return get_python_virtual_env();
    // }
    // std::string get_nodejs_package_manager() {
    //     return get_nodejs_package_manager();
    // }
    // std::string get_language_version(const std::string& language) {
    //     return get_language_version(language);
    // }
    // bool is_language_project(const std::string& language) {
    //     return is_language_project(language);
    // }

    // std::string get_container_name() {
    //     return get_container_name();
    // }
    // bool is_in_container() {
    //     return is_in_container();
    // }
    // std::string get_container_type() {
    //     return get_container_type();
    // }
    // bool is_in_docker() {
    //     return is_in_docker();
    // }
    // std::string get_docker_context() {
    //     return get_docker_context();
    // }
    // std::string get_docker_image() {
    //     return get_docker_image();
    // }
};
