...
struct JobControlJob {
  ...
  JobControlJob(int id, pid_t group_id, const std::vector<pid_t>& process_ids, 
    ...
    : job_id(id), pgid(group_id), pids(process_ids), command(cmd), 
      state(JobState::RUNNING), exit_status(0), notified(false) {}
};
...
class JobManager {
 public:
  ...
  // Get current job
  int get_current_job() const { return current_job; }
  
  // Get previous job
  int get_previous_job() const { return previous_job; }
  
  // Set last background PID
  void set_last_background_pid(pid_t pid) { last_background_pid = pid; }
  
  // Get last background PID
  pid_t get_last_background_pid() const { return last_background_pid; }
  
  // Cleanup finished jobs
  void cleanup_finished_jobs();
  
  // Set shell reference
  void set_shell(Shell* shell) { shell_ref = shell; }
