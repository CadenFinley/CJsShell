    ...
    double evaluateExpression(const std::string& expr) {
        ...
        while (std::getline(tokenStream, token, ' ')) {
            ...
            if (token == "sin") {
                ...
                values.push(std::sin(std::stod(arg) * M_PI / 180.0)); // Assuming degrees
            }
            else if (token == "cos") {
                std::string arg;
                std::getline(tokenStream, arg, ' ');
                values.push(std::cos(std::stod(arg) * M_PI / 180.0)); // Assuming degrees
            }
            else if (token == "tan") {
                std::string arg;
                std::getline(tokenStream, arg, ' ');
                values.push(std::tan(std::stod(arg) * M_PI / 180.0)); // Assuming degrees
            }
            else if (token == "sqrt") {
                std::string arg;
                std::getline(tokenStream, arg, ' ');
                ...
                std::getline(tokenStream, arg, ' ');
                double value = std::stod(arg);
                if (value <= 0) throw std::runtime_error("Natural log of non-positive number");
                ...
                while (!ops.empty() && getPrecedence(ops.top()) >= getPrecedence(token[0])) {
...
public:
    ...
    bool handleCommand(std::queue<std::string>& args) {
        ...
        if (cmd == "calc" || cmd == "c") {
            if (args.empty()) {
                ...
                std::cout << "Available functions: sin, cos, tan, sqrt, log, ln" << std::endl;
            ...
            if (memory.empty()) {
                ...
                for (std::map<std::string, double>::const_iterator it = memory.begin(); it != memory.end(); ++it) {
    ...
    void updateSetting(const std::string& key, const std::string& value) { 
        std::cout << "Calculator updated setting " << key << " to " << value << std::endl;
    }
};
