...
// g++ -std=c++17 -shared -dynamiclib -fPIC CJsZSH.cpp -o CJsZSH.dylib

class ZSHExecutorPlugin : public PluginInterface {
private:
...
public:
    ...
    std::string getDescription() const {
        return "A plugin to use ZSH shell command processor and executor.";
    }
    ...
    bool initialize() {
        ...
        if (system("which zsh > /dev/null 2>&1") != 0) {
            std::cerr << "ZSH shell is not available on this system" << std::endl;
            isInitialized = false;
            return false;
        }
        ...
        if (cmd == "event") {
            ...
            if (eventType == "main_process_command_processed") {
                ...
                if (!firstWord.empty() && firstWord != "cd") {
                    ...
                    if (settings.find("verbose") != settings.end() && settings["verbose"] == "true") {
                        std::cout << "ZSH Plugin captured command: " << capturedCommand << std::endl;
                    }
                }
            }
            ...
            //std::cout << "ZSH Plugin executing command: " << capturedCommand << std::endl;
            // This is our captured command - execute it via ZSH
            ...
            return executeZSHCommand(fullCommand);
        }
    ...
    void updateSetting(const std::string& key, const std::string& value) {
        settings[key] = value;
        std::cout << "ZSH Plugin setting updated - " << key << ": " << value << std::endl;
    }
};

IMPLEMENT_PLUGIN(ZSHExecutorPlugin)
